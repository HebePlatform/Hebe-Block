...
var NRS = (function(NRS, $) {
			...
			if (contacts && contacts.length) {
				...
				$.each(contacts, function (index, contact) {
					...
					rows += "<tr>" +
						...
						"<a class='btn btn-xs btn-default' href='#' data-toggle='modal' data-target='#send_money_modal' data-contact='" + String(contact.name).escapeHTML() + "'>" + $.t("send_hebe") + "</a>&nbsp;" +
	...
	NRS.forms.addContact = function($modal) {
		...
		if (/^\d+$/.test(data.name) || /^HEBE\-/i.test(data.name)) {
			return {
		...
		var address;
		if (/^HEBE\-/i.test(data.account_id)) {
			data.account_rs = data.account_id;
			address = new HebeAddress();
	...
	$("#update_contact_modal").on("show.bs.modal", function(e) {
		...
		if (!contactId && NRS.selectedContext) {
			...
			var dbKey = (/^HEBE\-/i.test(accountId) ? "accountRS" : "account");
			var dbQuery = {};

            ...
            firstNonMaskPos = null, $.each(mask.split(""), function(i, c) {
                ...
                function androidInputEvent() {
                    ...
                    if (oldVal && oldVal.length && oldVal.length > curVal.length) {
                        ...
                        var addressStart = curValUpper.indexOf('HEBE-', 5);
                        if (addressStart > 0) {
                            var insertedAddress = curValUpper.substr(addressStart, 24);
                            if (/HEBE\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{5}/.test(insertedAddress)) {
                ...
                function keydownEvent(e) {
                    ...
                    if (e.keyCode == 8) {
                        ...
                        if (settings.unmask !== false) {
                            //backspace, remove
                            if ((pos.begin == 0 && pos.end == 24) || (currentInput == "HEBE-____-____-____-_____" && pos.begin == 5)) {
                                input.val("");
                                $(this).trigger("unmask");
                                return;
                            }
                ...
                input.bind("keyup.remask", function(e) {
                    if (input.val().toLowerCase() == "hebe-") {
                        input.val("").mask("HEBE-****-****-****-*****")./*unbind(".remask").*/trigger("focus");
                    }
                }).bind("paste.remask", function(e) {
                    setTimeout(function() {
                        var newInput = input.val();
                        var pastedData = newInput.substring(4).toUpperCase();
                        if (/HEBE\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{5}/i.test(pastedData)) {
                            var newAddress = String(pastedData.match("HEBE\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{5}"));
                            input.val(newAddress);
                            checkVal(true);
                        } else if (/^HEBE\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{5}/i.test(newInput) || /^HEBE[A-Z0-9]{17}/i.test(newInput)) {
                            input.mask("HEBE-****-****-****-*****").trigger("checkRecipient")/*.unbind(".remask")*/;
                        }
                    }, 0);
                });

...
var NRS = (function (NRS, $, undefined) {
    ...
    NRS.listAccounts = function () {
        ...
        loginAccount.empty();
        if (NRS.getStrItem("savedHebeAccounts") && NRS.getStrItem("savedHebeAccounts") != "") {
            $('#login_account_container').show();
            $('#login_account_container_other').hide();
            var accounts = NRS.getStrItem("savedHebeAccounts").split(";");
    ...
    $("#loginButtons").find(".btn").click(function (e) {
        ...
        if (reader.is(':visible') && type != "scan") {
            NRS.scanQRCode(readerId, function () {
            }); // turn off scanning
        }
        if (type == "account") {
            NRS.listAccounts();
            $('#login_password').parent().hide();
            $('#login_aliases').parent().hide();
            ...
            $('#login_account_container_other').hide();
            $('#login_password').parent().hide();
            $('#login_aliases').parent().show();
        } else if (type == "scan" && !reader.is(':visible')) {
            NRS.scanQRCode(readerId, function (text) {
                var hebeAddress = new HebeAddress();
                if (hebeAddress.set(text)) {
    ...
    NRS.removeAccount = function (account) {
        var accounts = NRS.getStrItem("savedHebeAccounts").replace(account + ';', '');
        if (accounts == '') {
            NRS.removeItem('savedHebeAccounts');
        } else {
            NRS.setStrItem("savedHebeAccounts", accounts);
        }
        NRS.listAccounts();
    };

    function rememberAccount(account) {
        var accountsStr = NRS.getStrItem("savedHebeAccounts");
        if (!accountsStr) {
            NRS.setStrItem("savedHebeAccounts", account + ";");
            return;
        }
        ...
        NRS.setStrItem("savedHebeAccounts", accountsStr + account + ";");
    }
        ...
        if (isPassphraseLogin == 2) {
            ...
            NRS.sendRequest("getAlias", {
                ...
                if(response.errorCode){
                    ...
                    NRS.login(0,response.accountRS)
                }
            })
            ...
            NRS.sendRequest(accountRequest, requestVariable, function (response, data) {
                ...
                NRS.sendRequest("getAccountPublicKey", {
                    ...
                    if (NRS.getStrItem("savedHebeAccounts") && NRS.getStrItem("savedHebeAccounts") != "") {
                        ...
                        accounts = NRS.getStrItem("savedHebeAccounts").split(";");
                        $.each(accounts, function (index, account) {
                            if (account != '') {
    ...
    NRS.initPluginWarning = function () {
        if (NRS.activePlugins) {
            ...
            html += "Plugins are not sandboxed or restricted in any way and have full accesss to your client system including your Hebe passphrase.";
            html += "</p>";
            ...
            html += "you can loose your HEBE! In doubt don't run plugins with accounts ";
            html += "used to store larger amounts of HEBE now or in the future.";
            html += "</p>";
            html += "</div>";

...
var NRS = (function(NRS, $) {
    ...
    var coinToPair = function (op, coin) {
        return (op == "buy") ? "HEBE_" + coin : coin + "_HEBE";
    };

    var pairToCoin = function (pair) {
        if (pair.indexOf("HEBE_") == 0) {
            return pair.substring("HEBE_".length);
        }
        if (pair.indexOf("_HEBE") == pair.length - "_HEBE".length) {
            return pair.substring(0, pair.indexOf("_HEBE"));
        }
    ...
    var renderExchangeTable = function (op) {
        ...
        async.series(tasks, function (err, results) {
            var table = $("#p_shape_shift_" + op + "_hebe");
            if (err) {
        ...
        apiCall('recenttx/50', {}, 'GET', function (data) {
            ...
            if (data) {
                for (var i = 0; i < data.length; i++) {
                    ...
                    if (String(transaction.curIn).escapeHTML() != "HEBE" && String(transaction.curOut).escapeHTML() != "HEBE") {
                        continue;
                    }
                    rows += "<tr>";
    ...
    function renderHebeLimit() {
        apiCall('limit/hebe_btc', {}, 'GET', function (data) {
            NRS.logConsole("limit1 " + data.limit);
            if (data.limit) {
                $('#shape_shift_status').html('ok');
                $('#shape_shift_hebe_avail').html(String(data.limit).escapeHTML());
            }
        });
    }
        ...
        apiCall('getcoins', {}, 'GET', function (data) {
            ...
            for (var i = 0; i < inputFields.length; i++) {
                ...
                $.each(data, function (code, coin) {
                    if (code != 'HEBE' && coin['status'] == 'available') {
    ...
    $("#m_shape_shift_buy_modal").on("show.bs.modal", function (e) {
        ...
        $("#m_shape_shift_buy_title").html($.t("exchange_hebe_to_coin_shift", { coin: coin }));
        $("#m_shape_shift_buy_min").val(invoker.data("min"));
        $("#m_shape_shift_buy_min_coin").html("HEBE");
        $("#m_shape_shift_buy_max").val(invoker.data("max"));
        $("#m_shape_shift_buy_max_coin").html("HEBE");
        $("#m_shape_shift_buy_rate").val(invoker.data("rate"));
        $("#m_shape_shift_buy_rate_text").html("HEBE/" + coin);
    ...
    $("#m_send_amount_buy_modal").on("show.bs.modal", function (e) {
        ...
        var coin = pairToCoin(pair);
        NRS.logConsole("modal invoked pair " + pair + " coin " + coin);
        $("#m_send_amount_buy_title").html($.t("exchange_hebe_to_coin_send_amount", { coin: coin }));
        $("#m_send_amount_buy_withdrawal_amount_coin").html(coin);
        $("#m_send_amount_buy_rate_text").html("HEBE/" + coin);
    ...
    $('#m_send_amount_buy_withdrawal_amount, #m_send_amount_buy_withdrawal_address').change(function () {
        ...
        apiCall('sendamount', {
            ...
            try {
                ...
                // add 1 HEBE fee to make sure the net amount is what requested by shape shift
    ...
    $("#m_shape_shift_sell_modal").on("show.bs.modal", function (e) {
        ...
        var coin = pairToCoin(pair);
        NRS.logConsole("modal invoked pair " + pair + " coin " + coin);
        $("#m_shape_shift_sell_title").html($.t("exchange_coin_to_hebe_shift", { coin: coin }));
        ...
        async.waterfall([
            ...
            function(callback) {
                ...
                $("#m_shape_shift_sell_rate_text").html(coin + "/HEBE");
                $("#m_shape_shift_sell_fee").val(data.fee);
                $("#m_shape_shift_sell_fee_coin").html("HEBE");
    ...
    $("#").on("show.bs.modal", function (e) {
        ...
        var coin = pairToCoin(pair);
        NRS.logConsole("modal invoked pair " + pair + " coin " + coin);
        $("#m_send_amount_sell_title").html($.t("exchange_coin_to_hebe_send_amount", { coin: coin }));
        $("#m_send_amount_sell_rate_text").html("HEBE/" + coin);
        $("#m_send_amount_sell_fee_coin").html("HEBE");
        $("#m_send_amount_sell_withdrawal_amount_coin").html("HEBE");

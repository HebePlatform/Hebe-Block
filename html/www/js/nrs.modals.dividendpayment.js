...
var NRS = (function(NRS, $, undefined) {
    ...
    NRS.forms.dividendPayment = function($modal) {
        ...
        delete data.amountHEBEPerShare;
        return {
            "data": data
        };
    };
    ...
    $("#dividend_payment_amount_per_share, #dividend_payment_height").on("blur", function() {
        ...
        var amountHEBEPerShare = $modal.find("#dividend_payment_amount_per_share").val();
        ...
        if (amountHEBEPerShare && /^\d+$/.test(height)) {
            NRS.getAssetAccounts(NRS.getCurrentAsset().asset, height,
                function (response) {
                    ...
                    var priceNQT = new BigInteger(NRS.calculatePricePerWholeQNT(NRS.convertToNQT(amountHEBEPerShare), NRS.getCurrentAsset().decimals));
                    var totalHEBE = NRS.calculateOrderTotal(totalQuantityQNT, priceNQT);
                    $callout.html($.t("dividend_payment_info_preview_success",
                            {
                                "amountHEBE": totalHEBE,

...
var NRS = (function(NRS, $) {
			...
			html += "<table width='100%' style='table-layout:fixed;'>";
				html += "<tr>";
					...
					html += "<td>";
						...
						html += "<div class='price'><strong>" + NRS.formatAmount(good.priceNQT) + " HEBE</strong></div>";
	...
	NRS.getMarketplacePurchaseHTML = function(purchase, showBuyer) {
		...
		return "<div data-purchase='" + NRS.escapeRespStr(purchase.purchase) + "' " + (purchase.unconfirmed ? "class='tentative'" : "") + "><div style='float:right;color: #999999;background:white;padding:5px;border:1px solid #ccc;border-radius:3px'>" +
			...
			"<tr><td><strong>" + $.t("price") + "</strong>:</td><td>" + NRS.formatAmount(purchase.priceNQT) + " HEBE</td></tr>" +
	...
	NRS.dgs_search_main = function(callback) {
		...
		$(".dgs_search input[name=q]").val("").trigger("unmask").mask("HEBE-****-****-****-*****");
         ...
         var rows = "";
			if (response.goods && response.goods.length) {
				...
				for (var i = 0; i < response.goods.length; i++) {
					...
					rows += "<td class='price numeric'>" + NRS.formatAmount(good.priceNQT, false, false, priceDecimals) + " HEBE</td>";
	...
	NRS.forms.dgsPriceChangeComplete = function(response, data) {
		...
		$("#my_dgs_listings_table").find("tr[data-goods=" + String(data.goods).escapeHTML() + "]").addClass("tentative").find(".price").html(NRS.formatAmount(data.priceNQT) + " HEBE");
	};
			...
			if (response.errorCode) {
				...
				NRS.sendRequest("getDGSGood", {
					...
					if (good.errorCode) {
						...
						output += "<tr><th><strong>" + $.t("price") + "</strong>:</th><td>" + NRS.formatAmount(response.priceNQT) + " HEBE</td></tr>";
						...
						if (type == "dgs_refund_modal" || type == "dgs_delivery_modal" || type == "dgs_feedback_modal") {
							...
							if (response.quantity != "1") {
								...
								output += "<tr><th><strong>" + $.t("total") + "</strong>:</th><td>" + orderTotal + " HEBE</td></tr>";
							}
							if (response.discountNQT && (type == "dgs_refund_modal" || type == "dgs_feedback_modal")) {
								output += "<tr><th><strong>" + $.t("discount") + "</strong>:</th><td>" + NRS.formatAmount(response.discountNQT) + " HEBE</td></tr>";
							}
						}
						...
						if (type == "dgs_view_refund_modal") {
							output += "<tr><th><strong>" + $.t("refund_price") + "</strong>:</th><td>" + NRS.formatAmount(response.refundNQT) + " HEBE</td></tr>";
						}
						...
						if (type == "dgs_refund_modal") {
							...
							$("#dgs_refund_refund").val(NRS.convertToHEBE(refund));
			...
			if (response.errorCode) {
				...
				NRS.sendRequest("getDGSGoodsPurchases+", {
					...
					if (type == "dgs_quantity_change_modal") {
						...
						$("#dgs_price_change_current_price, #dgs_price_change_price").val(NRS.convertToHEBE(response.priceNQT).escapeHTML());
					} else if (type == "dgs_purchase_modal") {
						$modal.find("input[name=recipient]").val(response.sellerRS);

						$("#dgs_purchase_price").val(NRS.escapeRespStr(response.priceNQT));
						$("#dgs_total_purchase_price").html(NRS.formatAmount(response.priceNQT) + " HEBE");
						...
						$("#dgs_purchase_quantity").on("change", function() {
							...
							$("#dgs_total_purchase_price").html(NRS.formatAmount(totalNQT) + " HEBE");
						});
					}
				});
			}

...
var NRS = (function (NRS, $, undefined) {
    ...
    NRS.sendRequest = function (requestType, data, callback, options) {
        ...
        try {
            var hebeFields = [
                ["feeHEBE", "feeNQT"],
                ["amountHEBE", "amountNQT"],
                ["priceHEBE", "priceNQT"],
                ["refundHEBE", "refundNQT"],
                ["discountHEBE", "discountNQT"],
                ["phasingQuorumHEBE", "phasingQuorum"],
                ["phasingMinBalanceHEBE", "phasingMinBalance"],
                ["controlQuorumHEBE", "controlQuorum"],
                ["controlMinBalanceHEBE", "controlMinBalance"],
                ["controlMaxFeesHEBE", "controlMaxFees"],
                ["minBalanceHEBE", "minBalance"],
                ["shufflingAmountHEBE", "amount"],
                ["monitorAmountHEBE", "amount"],
                ["monitorThresholdHEBE", "threshold"]
            ];

            for (i = 0; i < hebeFields.length; i++) {
                var hebeField = hebeFields[i][0];
                var nqtField = hebeFields[i][1];
                if (hebeField in data) {
                    data[nqtField] = NRS.convertToNQT(data[hebeField]);
                    delete data[hebeField];
                }
            }
        } catch (err) {
            callback({
            ...
            if (maxFees > 0 && new BigInteger(data.feeNQT).compareTo(new BigInteger(phasingControl.maxFees)) > 0) {
                callback({
                    ...
                    "errorDescription": $.t("error_fee_exceeds_max_account_control_fee", {
                        "maxFee": NRS.convertToHEBE(phasingControl.maxFees)
                    })
                });
                return;
            }
        ...
        if (requestType == "getAccountId") {
            ...
            var accountRS = "";
            if (hebeAddress.set(accountId)) {
                accountRS = hebeAddress.toString();
            }
            callback({
                "account": accountId,
                "accountRS": accountRS
    ...
    NRS.processAjaxRequest = function (requestType, data, callback, options) {
        ...
        if (options.remoteNode) {
            url = options.remoteNode.getUrl() + "/hebe";
        } else {

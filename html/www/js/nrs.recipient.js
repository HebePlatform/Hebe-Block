...
var NRS = (function(NRS, $) {
	NRS.automaticallyCheckRecipient = function() {
		...
		$recipientFields.on("checkRecipient", function() {
			...
			if (value && value != "HEBE-____-____-____-_____") {
			...
			if (response.publicKey) {
				if (response.name){
					result = {
						...
						"message": $.t("recipient_info_with_name", {
							...
							"hebe": NRS.formatAmount(response.unconfirmedBalanceNQT, false, true)
						}),
						"account": response
					};
				}
				...
				if (response.errorCode) {
					if (response.errorCode == 4) {
						result = {
							"type": "danger",
							"message": $.t("recipient_malformed") + (!/^(HEBE\-)/i.test(accountId) ? " " + $.t("recipient_alias_suggestion") : ""),
							"account": null
						};
	...
	NRS.checkRecipient = function(account, modal) {
		...
		if (/^(HEBE\-)?[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+/i.test(account)) {

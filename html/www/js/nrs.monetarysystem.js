...
var NRS = (function (NRS, $, undefined) {
    ...
    search.on("submit", function (e, data) {
        ...
        $("#buy_currency_with_hebe").html($.t("buy_currency_param", { currency: currencyCode}));
        $("#buy_currency_offers").html($.t("offers_to_buy_currency_param", { currency: currencyCode}));
        $("#sell_currency_with_hebe").html($.t("sell_currency_param", { currency: currencyCode}));
        ...
        async.waterfall([
            ...
            function(callback) {
                ...
                if (NRS.accountInfo.unconfirmedBalanceNQT == "0") {
                    $("#ms_your_hebe_balance").html("0");
                } else {
                    $("#ms_your_hebe_balance").html(NRS.formatAmount(NRS.accountInfo.unconfirmedBalanceNQT));
                }
                NRS.pageLoaded();
                callback(null);
            }
        ...
        NRS.sendRequest("getCurrencyFounders", {
            ...
            if (response.founders && response.founders.length) {
                ...
                for (i = 0; i < response.founders.length; i++) {
                    ...
                    "<td>" + NRS.convertToHEBE(amountPerUnitNQT) + "</td>" +
                    "<td>" + NRS.convertToHEBE(amountPerUnitNQT.multiply(new BigInteger(NRS.convertToQNTf(resSupply, decimals)))) + "</td>" +
            ...
            rows += "<tr>" +
            "<td><b>Totals</b></td>" +
            "<td>" + NRS.convertToHEBE(totalAmountReserved) + "</td>" +
            "<td>" + NRS.convertToHEBE(totalAmountReserved.multiply(new BigInteger(NRS.convertToQNTf(resSupply, decimals)))) + "</td>" +
    ...
    $("#currency_order_modal").on("show.bs.modal", function (e) {
        ...
        var totalHEBE = NRS.formatAmount(totalNQT, false, true);
        ...
        $("#currency_order_total").html(totalHEBE + " HEBE");
    ...
    unitFields.on("change", function() {
        ...
        NRS.sendRequest("getAvailableTo" + NRS.initialCaps(orderType), {
            ...
            rateField.val(rate);
            var amount = NRS.convertToHEBE(response.amountNQT);
    ...
    $("#reserve_currency_modal").on("show.bs.modal", function (e) {
        ...
        NRS.sendRequest("getCurrency+", {
            ...
            if (response && !response.errorDescription) {
                ...
                $("#reserve_currency_minReserve_text").html(NRS.formatQuantity(NRS.convertToHEBE(minReservePerUnitNQT.multiply(new BigInteger(resSupply))), decimals));
                ...
                $("#reserve_currency_currentReserve_text").html(NRS.formatQuantity(NRS.convertToHEBE(currentReservePerUnitNQT.multiply(new BigInteger(resSupply))), decimals));
    ...
    reserveCurrencyAmount.blur(function () {
        ...
        var roundUnitAmountNQT = NRS.convertToNQT(NRS.amountToPrecision(NRS.convertToHEBE(unitAmountNQT), decimals));
        $("#reserve_currency_total").val(NRS.convertToHEBE(roundUnitAmountNQT));
        reserveCurrencyAmount.val(NRS.convertToHEBE(new BigInteger(roundUnitAmountNQT).multiply(new BigInteger(resSupply)).toString()));
    });
    ...
    $("#claim_currency_modal").on("show.bs.modal", function (e) {
        ...
        NRS.sendRequest("getCurrency", {
            ...
            $("#claimRate").html(NRS.formatAmount(currentReservePerUnitNQT) + " [HEBE/" + currencyCode + "]");
        });

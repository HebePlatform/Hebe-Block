...
var NRS = (function(NRS, $) {
	...
	NRS.processAccountModalData = function(account) {
		if (account.unconfirmedBalanceNQT == "0") {
			...
			$("#user_info_modal_account_balance").html(NRS.formatAmount(account.unconfirmedBalanceNQT) + " HEBE");
		}
        ...
        NRS.sendRequest("getBlockchainTransactions", {
			...
			if (response.transactions && response.transactions.length) {
				...
				for (var i = 0; i < response.transactions.length; i++) {
                    ...
                    var receiving;
					if (/^HEBE\-/i.test(String(NRS.userInfoModal.user))) {

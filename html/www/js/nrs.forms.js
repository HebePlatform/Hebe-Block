...
var NRS = (function(NRS, $) {
    ...
    NRS.submitForm = function($modal, $btn) {
		...
		$form.find(":input").each(function() {
			if ($(this).is(":invalid")) {
				var error = "";
				var name = String($(this).attr("name")).replace("HEBE", "").replace("NQT", "").capitalize();
				var value = $(this).val();
		...
		if (data.recipient) {
			...
			if (/^\d+$/.test(data.recipient)) {
				...
				NRS.unlockForm($modal, $btn);
				return;
			} else if (!/^HEBE\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+/i.test(data.recipient)) {
				var convertedAccountId = $modal.find("input[name=converted_account_id]").val();
				if (!convertedAccountId || (!/^\d+$/.test(convertedAccountId) && !/^HEBE\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+/i.test(convertedAccountId))) {
		...
		if (!NRS.showedFormWarning) {
			if ("amountHEBE" in data && NRS.settings["amount_warning"] && NRS.settings["amount_warning"] != "0") {
				try {
					var amountNQT = NRS.convertToNQT(data.amountHEBE);
				} catch (err) {
				...
				if (new BigInteger(amountNQT).compareTo(new BigInteger(NRS.settings["amount_warning"])) > 0) {
					...
					$form.find(".error_message").html($.t("error_max_amount_warning", {
						"hebe": NRS.formatAmount(NRS.settings["amount_warning"])
					})).show();
					if (formErrorFunction) {
						formErrorFunction(false, data);
					}
			...
			if ("feeHEBE" in data && NRS.settings["fee_warning"] && NRS.settings["fee_warning"] != "0") {
				try {
					var feeNQT = NRS.convertToNQT(data.feeHEBE);
				} catch (err) {
				...
				if (new BigInteger(feeNQT).compareTo(new BigInteger(NRS.settings["fee_warning"])) > 0) {
					...
					$form.find(".error_message").html($.t("error_max_fee_warning", {
						"hebe": NRS.formatAmount(NRS.settings["fee_warning"])
					})).show();
					if (formErrorFunction) {
						formErrorFunction(false, data);
					}
			...
			var convertHEBEFields = ["phasingQuorumHEBE", "phasingMinBalanceHEBE"];
			$.each(convertHEBEFields, function(key, field) {
				if (field in data) {
					try {
    ...
    function updateFee(modal, feeNQT) {
        ...
        fee.val(NRS.convertToHEBE(feeNQT));
        var recalcIndicator = $("#" + modal.attr('id').replace('_modal', '') + "_recalc");

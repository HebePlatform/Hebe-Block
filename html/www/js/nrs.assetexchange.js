...
var NRS = (function (NRS, $, undefined) {
    ...
    NRS.forms.addAssetBookmark = function ($modal) {
        ...
        if (!/^\d+$/.test(data.id) && !/^HEBE\-/i.test(data.id)) {
            return {
                "error": $.t("error_asset_or_account_id_invalid")
            };
        }

        if (/^HEBE\-/i.test(data.id)) {
    ...
    NRS.loadAsset = function (asset, refresh) {
        ...
        if (!refresh) {
            ...
            $("#sell_asset_for_hebe").html($.t("sell_asset_for_hebe", {
                "assetName": NRS.escapeRespStr(asset.name)
            }));
            $("#buy_asset_with_hebe").html($.t("buy_asset_with_hebe", {
                "assetName": NRS.escapeRespStr(asset.name)
            }));
        ...
        if (NRS.accountInfo.unconfirmedBalanceNQT == "0") {
            $("#your_hebe_balance").html("0");
            $("#buy_automatic_price").addClass("zero").removeClass("nonzero");
        } else {
            $("#your_hebe_balance").html(NRS.formatAmount(NRS.accountInfo.unconfirmedBalanceNQT));
    ...
    $("#asset_exchange_bid_orders_table tbody, #asset_exchange_ask_orders_table tbody").on("click", "td", function (e) {
        ...
        try {
            ...
            $("#" + type + "_asset_total").val(NRS.convertToHEBE(totalNQT));
        } catch (err) {
            return;
        }
    ...
    $("#sell_automatic_price, #buy_automatic_price").on("click", function () {
        try {
            ...
            if (price.cmp(new Big("0")) <= 0) {
                ...
                assetPrice.val(NRS.convertToHEBE(price.toString()));
            }
            ...
            assetTotal.val(NRS.convertToHEBE(total.toString()));
            assetTotal.css({
                "background": "",
                "color": ""
            });
    ...
    $("#asset_order_modal").on("show.bs.modal", function (e) {
        ...
        try {
            ...
            var priceNQT = new BigInteger(NRS.calculatePricePerWholeQNT(NRS.convertToNQT(String($("#" + orderType + "_asset_price").val())), currentAsset.decimals));
            var totalHEBE = NRS.formatAmount(NRS.calculateOrderTotalNQT(quantityQNT, priceNQT, currentAsset.decimals), false, true);
        } catch (err) {
        ...
        if (orderType == "buy") {
            description = $.t("buy_order_description", {
                ...
                "hebe": NRS.formatAmount(priceNQTPerWholeQNT)
            });
            tooltipTitle = $.t("buy_order_description_help", {
                "hebe": NRS.formatAmount(priceNQTPerWholeQNT, false, true),
        ...
        $("#asset_order_total").html(totalHEBE + " HEBE");

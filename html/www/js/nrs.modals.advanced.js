...
var NRS = (function(NRS, $, undefined) {
	...
	NRS.initAdvancedModalFormValues = function($modal) {
		...
		context = {
			...
			inputName: "phasingQuorumHEBE",
			addonText: "HEBE",
			addonI18n: "hebe_unit"
		};
		$elems = NRS.initModalUIElement($modal, '.approve_transaction_amount_hebe', 'simple_input_with_addon_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		...
		context['inputName'] = 'phasingMinBalanceHEBE';
		context['addonText'] = 'HEBE';
		context['addonI18n'] = 'hebe_unit';
		$elems = NRS.initModalUIElement($modal, '.approve_min_balance_hebe', 'simple_input_with_addon_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		$elems.hide();

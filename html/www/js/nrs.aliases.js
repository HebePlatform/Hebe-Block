...
var NRS = (function (NRS, $, undefined) {
    ...
    NRS.forms.sellAlias = function ($modal) {
        ...
        if (data.modal == "cancel_alias_sale") {
            ...
            if (data.recipient == NRS.constants.GENESIS_RS) {
                if (!data.priceHEBE || data.priceHEBE == "0") {
                    return {
                        "error": $.t("error_not_specified", {
        ...
        if (data.priceNQT == "0") {
            ...
            $.growl(
                $.t("success_alias_sell", {
                    ...
                    "price": NRS.convertToHEBE(data.priceNQT).escapeHTML()
                }), {
                    "type": "success"
                }
            );
        ...
        NRS.sendRequest("getAlias", {
            ...
            if (response.errorCode) {
                ...
                if (!("priceNQT" in response)) {
                    ...
                    $modal.find("input[name=amountHEBE]").val(NRS.convertToHEBE(response.priceNQT)).prop("readonly", true);
                }
            }
        }, { isAsync: false });
    });
    ...
    NRS.forms.buyAliasError = function () {
        $("#buy_alias_modal").find("input[name=priceHEBE]").prop("readonly", false);
    };
    ...
    NRS.forms.buyAliasComplete = function (response, data) {
        ...
        $.growl(
            $.t("success_alias_buy", {
                "alias_name": data.aliasName,
                "price": NRS.convertToHEBE(data.amountNQT).escapeHTML()
            }), {
                "type": "success"
            }
        );
        ...
        if (alias) {
            ...
            NRS.sendRequest("getAlias", {
                ...
                if (response.errorCode) {
                    ...
                    } else if ((aliasURI = /acct:(.*)@hebe/.exec(response.aliasURI)) || (aliasURI = /nacc:(.*)/.exec(response.aliasURI))) {
    ...
    NRS.forms.setAlias = function ($modal) {
        ...
        if (data.type == "account") {
            if (!(/acct:(.*)@hebe/.test(data.aliasURI)) && !(/nacc:(.*)/.test(data.aliasURI))) {
                if (/^(HEBE\-)/i.test(data.aliasURI)) {
                    ...
                    if (!address.set(data.aliasURI)) {
                        ...
                        data.aliasURI = "acct:" + data.aliasURI + "@hebe";
                    }
    ...
    function setAliasType(type, uri) {
        ...
        if (type == "uri") {
            ...
            registerAliasUri.val("").mask("HEBE-****-****-****-*****");

            if (uri) {
                var match = uri.match(/acct:(.*)@hebe/i);
                if (!match) {
                ...
                } else if (!/^HEBE\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{5}/i.test(uri)) {
                    uri = NRS.accountRS;
                }
                    ...
                    if ("priceNQT" in response) {
                        if (response.buyer == NRS.account) {
                            message = $.t("alias_sale_direct_offer", {
                                "hebe": NRS.formatAmount(response.priceNQT)

...
import hebe.db.PrunableDbTable;
...
public final class ShufflingParticipant {
    ...
    void setData(byte[][] data, int timestamp) {
        if (data != null && HEBE.getEpochTime() - timestamp < Constants.MAX_PRUNABLE_LIFETIME && getData() == null) {
            shufflingDataTable.insert(new ShufflingData(shufflingId, accountId, data, timestamp, HEBE.getBlockchain().getHeight()));
        }
    }

...
import hebe.util.ReadWriteUpdateLock;
 ...
 *   CREATE ALIAS FTL_CREATE_INDEX FOR "hebe.db.FullTextTrigger.createIndex"
 *       CALL FTL_CREATE(schema, table, columnList)
 *   CREATE ALIAS FTL_DROP_INDEX FOR "hebe.db.FullTextTrigger.dropIndex"
 *       CALL FTL_DROP(schema, table)
 *   CREATE ALIAS FTL_SEARCH FOR "hebe.db.FullTextTrigger.search"
 *       CALL FTL_SEARCH(schema, table, query, limit, offset)
 *
 * FTL_CREATE_INDEX is called to create a fulltext index for a table.  It is
 * provided as a convenience for use in HebeDbVersion when creating a new index
 * after the database has been created.
 *
 * FTL_DROP_INDEX is called to drop a fulltext index for a table.  It is
 * provided as a convenience for use in HebeDbVersion when dropping a table
 * after the database has been created.
 *
 ...
 *   CREATE TRIGGER trigger_name AFTER INSERT,UPDATE,DELETE ON table_name FOR EACH ROW CALL "hebe.db.FullTextTrigger"
 */
public class FullTextTrigger implements Trigger, TransactionalDb.TransactionCallback {
     ...
     *
     * This method should be called from HebeDbVersion when performing the database version update

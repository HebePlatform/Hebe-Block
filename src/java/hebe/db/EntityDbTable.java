...
public abstract class EntityDbTable<T> extends DerivedDbTable {
    ...
    public void checkAvailable(int height) {
        if (multiversion) {
            int minRollBackHeight = isPersistent() && HEBE.getBlockchainProcessor().isScanning() ?
                    Math.max(HEBE.getBlockchainProcessor().getInitialScanHeight() - Constants.MAX_ROLLBACK, 0)
                    : HEBE.getBlockchainProcessor().getMinRollbackHeight();
                if(minRollBackHeight>0 && height==0){
    ...
    private boolean doesNotExceed(int height) {
        return HEBE.getBlockchain().getHeight() <= height && ! (isPersistent() && HEBE.getBlockchainProcessor().isScanning());
    }

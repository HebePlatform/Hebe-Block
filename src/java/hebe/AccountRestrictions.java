...
import hebe.HebeException.AccountControlException;
...
public final class AccountRestrictions {
    ...
    public static final class PhasingOnly {
        ...
        private void checkTransaction(Transaction transaction, boolean validatingAtFinish) throws AccountControlException {
            if (!validatingAtFinish && maxFees > 0 && Math.addExact(transaction.getFeeNQT(), PhasingPoll.getSenderPhasedTransactionFees(transaction.getSenderId())) > maxFees) {
                throw new AccountControlException(String.format("Maximum total fees limit of %f HEBE exceeded", ((double)maxFees)/Constants.ONE_HEBE));
            }
            ...
            if (!validatingAtFinish) {
                int duration = phasingAppendix.getFinishHeight() - HEBE.getBlockchain().getHeight();
    ...
    static void checkTransaction(Transaction transaction, boolean validatingAtFinish) throws HebeException.NotCurrentlyValidException {
        Account senderAccount = Account.getAccount(transaction.getSenderId());
        if (senderAccount == null) {
            throw new HebeException.NotCurrentlyValidException("Account " + Long.toUnsignedString(transaction.getSenderId()) + " does not exist yet");
        }

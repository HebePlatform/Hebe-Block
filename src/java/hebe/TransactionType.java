...
import hebe.Account.ControlType;
import hebe.AccountLedger.LedgerEvent;
import hebe.Attachment.AbstractAttachment;
import hebe.VoteWeighting.VotingModel;
import hebe.HebeException.ValidationException;
...
public abstract class TransactionType {
    ...
    abstract Attachment.AbstractAttachment parseAttachment(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException;

    abstract Attachment.AbstractAttachment parseAttachment(JSONObject attachmentData) throws HebeException.NotValidException;

    abstract void validateAttachment(Transaction transaction) throws HebeException.ValidationException;
    ...
    public static abstract class Payment extends TransactionType {
        ...
        public static final TransactionType ORDINARY = new Payment() {
            ...
            @Override
            Attachment.EmptyAttachment parseAttachment(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException {
                return Attachment.ORDINARY_PAYMENT;
            }

            @Override
            Attachment.EmptyAttachment parseAttachment(JSONObject attachmentData) throws HebeException.NotValidException {
                return Attachment.ORDINARY_PAYMENT;
            }

            @Override
            void validateAttachment(Transaction transaction) throws HebeException.ValidationException {
                if (transaction.getAmountNQT() <= 0 || transaction.getAmountNQT() >= Constants.MAX_BALANCE_NQT) {
                    throw new HebeException.NotValidException("Invalid ordinary payment");
                }
            }
    ...
    public static abstract class Messaging extends TransactionType {
        ...
        public final static TransactionType ARBITRARY_MESSAGE = new Messaging() {
            ...
            void validateAttachment(Transaction transaction) throws HebeException.ValidationException {
                ...
                if (transaction.getAmountNQT() != 0) {
                    throw new HebeException.NotValidException("Invalid arbitrary message: " + attachment.getJSONObject());
                }
                if (transaction.getRecipientId() == Genesis.CREATOR_ID && HEBE.getBlockchain().getHeight() > Constants.MONETARY_SYSTEM_BLOCK) {
                    throw new HebeException.NotValidException("Sending messages to Genesis not allowed.");
                }
            }
        ...
        public static final TransactionType ALIAS_ASSIGNMENT = new Messaging() {
            ...
            private final Fee ALIAS_FEE = new Fee.SizeBasedFee(2 * Constants.ONE_HEBE, 2 * Constants.ONE_HEBE, 32) {
                @Override
            ...
            @Override
            Attachment.MessagingAliasAssignment parseAttachment(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException {
                return new Attachment.MessagingAliasAssignment(buffer, transactionVersion);
            }

            @Override
            Attachment.MessagingAliasAssignment parseAttachment(JSONObject attachmentData) throws HebeException.NotValidException {
            ...
            @Override
            boolean isBlockDuplicate(Transaction transaction, Map<TransactionType, Map<String, Integer>> duplicates) {
                return HEBE.getBlockchain().getHeight() > Constants.SHUFFLING_BLOCK
            ...
            void validateAttachment(Transaction transaction) throws HebeException.ValidationException {
                ...
                if (attachment.getAliasName().length() == 0
                    ...
                    throw new HebeException.NotValidException("Invalid alias assignment: " + attachment.getJSONObject());
                }
                String normalizedAlias = attachment.getAliasName().toLowerCase();
                for (int i = 0; i < normalizedAlias.length(); i++) {
                    if (Constants.ALPHABET.indexOf(normalizedAlias.charAt(i)) < 0) {
                        throw new HebeException.NotValidException("Invalid alias name: " + normalizedAlias);
                    }
                }
                Alias alias = Alias.getAlias(normalizedAlias);
                if (alias != null && alias.getAccountId() != transaction.getSenderId()) {
                    throw new HebeException.NotCurrentlyValidException("Alias already owned by another account: " + normalizedAlias);
                }
            }
        ...
        public static final TransactionType ALIAS_SELL = new Messaging() {
            ...
            @Override
            Attachment.MessagingAliasSell parseAttachment(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException {
                return new Attachment.MessagingAliasSell(buffer, transactionVersion);
            }

            @Override
            Attachment.MessagingAliasSell parseAttachment(JSONObject attachmentData) throws HebeException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws HebeException.ValidationException {
                if (transaction.getAmountNQT() != 0) {
                    throw new HebeException.NotValidException("Invalid sell alias transaction: " +
                ...
                if (aliasName == null || aliasName.length() == 0) {
                    throw new HebeException.NotValidException("Missing alias name");
                }
                long priceNQT = attachment.getPriceNQT();
                if (priceNQT < 0 || priceNQT > Constants.MAX_BALANCE_NQT) {
                    throw new HebeException.NotValidException("Invalid alias sell price: " + priceNQT);
                }
                if (priceNQT == 0) {
                    if (Genesis.CREATOR_ID == transaction.getRecipientId()) {
                        throw new HebeException.NotValidException("Transferring aliases to Genesis account not allowed");
                    } else if (transaction.getRecipientId() == 0) {
                        throw new HebeException.NotValidException("Missing alias transfer recipient");
                    }
                }
                final Alias alias = Alias.getAlias(aliasName);
                if (alias == null) {
                    throw new HebeException.NotCurrentlyValidException("No such alias: " + aliasName);
                } else if (alias.getAccountId() != transaction.getSenderId()) {
                    throw new HebeException.NotCurrentlyValidException("Alias doesn't belong to sender: " + aliasName);
                }
                if (transaction.getRecipientId() == Genesis.CREATOR_ID) {
                    throw new HebeException.NotValidException("Selling alias to Genesis not allowed");
                }
            }
        ...
        public static final TransactionType ALIAS_BUY = new Messaging() {
            ...
            @Override
            Attachment.MessagingAliasBuy parseAttachment(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException {
                return new Attachment.MessagingAliasBuy(buffer, transactionVersion);
            }

            @Override
            Attachment.MessagingAliasBuy parseAttachment(JSONObject attachmentData) throws HebeException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws HebeException.ValidationException {
                ...
                if (alias == null) {
                    ...
                    throw new HebeException.NotCurrentlyValidException("Alias is owned by account other than recipient: "
                            + Long.toUnsignedString(alias.getAccountId()));
                }
                Alias.Offer offer = Alias.getOffer(alias);
                if (offer == null) {
                    throw new HebeException.NotCurrentlyValidException("Alias is not for sale: " + aliasName);
                }
                if (transaction.getAmountNQT() < offer.getPriceNQT()) {
                    ...
                    throw new HebeException.NotCurrentlyValidException(msg);
                }
                if (offer.getBuyerId() != 0 && offer.getBuyerId() != transaction.getSenderId()) {
                    throw new HebeException.NotCurrentlyValidException("Wrong buyer for " + aliasName + ": "
        ...
        public static final TransactionType ALIAS_DELETE = new Messaging() {
            ...
            @Override
            Attachment.MessagingAliasDelete parseAttachment(final ByteBuffer buffer, final byte transactionVersion) throws HebeException.NotValidException {
                return new Attachment.MessagingAliasDelete(buffer, transactionVersion);
            }

            @Override
            Attachment.MessagingAliasDelete parseAttachment(final JSONObject attachmentData) throws HebeException.NotValidException {
            ...
            @Override
            void validateAttachment(final Transaction transaction) throws HebeException.ValidationException {
                final Attachment.MessagingAliasDelete attachment =
                        (Attachment.MessagingAliasDelete) transaction.getAttachment();
                final String aliasName = attachment.getAliasName();
                if (aliasName == null || aliasName.length() == 0) {
        ...
        public final static TransactionType POLL_CREATION = new Messaging() {
            ...
            private final Fee POLL_OPTIONS_FEE = new Fee.SizeBasedFee(10 * Constants.ONE_HEBE, Constants.ONE_HEBE, 1) {
                @Override
                public int getSize(TransactionImpl transaction, Appendix appendage) {
                    int numOptions = ((Attachment.MessagihebeollCreation)appendage).getPollOptions().length;
                    return numOptions <= 19 ? 0 : numOptions - 19;
                }
            };

            private final Fee POLL_SIZE_FEE = new Fee.SizeBasedFee(0, 2 * Constants.ONE_HEBE, 32) {
                @Override
                public int getSize(TransactionImpl transaction, Appendix appendage) {
                    Attachment.MessagihebeollCreation attachment = (Attachment.MessagihebeollCreation)appendage;
                    int size = attachment.getPollName().length() + attachment.getPollDescription().length();
                    for (String option : ((Attachment.MessagihebeollCreation)appendage).getPollOptions()) {
                        size += option.length();
                    }
            ...
            @Override
            Attachment.MessagihebeollCreation parseAttachment(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException {
                return new Attachment.MessagihebeollCreation(buffer, transactionVersion);
            }

            @Override
            Attachment.MessagihebeollCreation parseAttachment(JSONObject attachmentData) throws HebeException.NotValidException {
                return new Attachment.MessagihebeollCreation(attachmentData);
            }

            @Override
            void applyAttachment(Transaction transaction, Account senderAccount, Account recipientAccount) {
                Attachment.MessagihebeollCreation attachment = (Attachment.MessagihebeollCreation) transaction.getAttachment();
            ...
            void validateAttachment(Transaction transaction) throws HebeException.ValidationException {
                ...
                if (attachment.getPollName().length() > Constants.MAX_POLL_NAME_LENGTH
                        ...
                        || optionsCount == 0) {
                    throw new HebeException.NotValidException("Invalid poll attachment: " + attachment.getJSONObject());
                }

                if (attachment.getMinNumberOfOptions() < 1
                        || attachment.getMinNumberOfOptions() > optionsCount) {
                    throw new HebeException.NotValidException("Invalid min number of options: " + attachment.getJSONObject());
                }
                ...
                if (attachment.getMaxNumberOfOptions() < 1
                    ...
                    throw new HebeException.NotValidException("Invalid max number of options: " + attachment.getJSONObject());
                }

                for (int i = 0; i < optionsCount; i++) {
                    if (attachment.getPollOptions()[i].length() > Constants.MAX_POLL_OPTION_LENGTH
                            || attachment.getPollOptions()[i].isEmpty()) {
                        throw new HebeException.NotValidException("Invalid poll options length: " + attachment.getJSONObject());
                    }
                }
                ...
                if (attachment.getMinRangeValue() < Constants.MIN_VOTE_VALUE || attachment.getMaxRangeValue() > Constants.MAX_VOTE_VALUE
                    ...
                    throw new HebeException.NotValidException("Invalid range: " + attachment.getJSONObject());
                }
                ...
                if (attachment.getFinishHeight() <= attachment.getFinishValidationHeight(transaction) + 1
                    ...
                    throw new HebeException.NotCurrentlyValidException("Invalid finishing height" + attachment.getJSONObject());
                }

                if (! attachment.getVoteWeighting().acceptsVotes() || attachment.getVoteWeighting().getVotingModel() == VoteWeighting.VotingModel.HASH) {
                    throw new HebeException.NotValidException("VotingModel " + attachment.getVoteWeighting().getVotingModel() + " not valid for regular polls");
                }
        ...
        public final static TransactionType VOTE_CASTING = new Messaging() {
            ...
            @Override
            Attachment.MessagingVoteCasting parseAttachment(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException {
                return new Attachment.MessagingVoteCasting(buffer, transactionVersion);
            }

            @Override
            Attachment.MessagingVoteCasting parseAttachment(JSONObject attachmentData) throws HebeException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws HebeException.ValidationException {
                ...
                if (attachment.getPollId() == 0 || attachment.getPollVote() == null
                    ...
                    throw new HebeException.NotValidException("Invalid vote casting attachment: " + attachment.getJSONObject());
                }
                ...
                if (poll == null) {
                    throw new HebeException.NotCurrentlyValidException("Invalid poll: " + Long.toUnsignedString(attachment.getPollId()));
                }

                if (Vote.getVote(pollId, transaction.getSenderId()) != null) {
                    throw new HebeException.NotCurrentlyValidException("Double voting attempt");
                }

                if (poll.getFinishHeight() <= attachment.getFinishValidationHeight(transaction)) {
                    throw new HebeException.NotCurrentlyValidException("Voting for this poll finishes at " + poll.getFinishHeight());
                }
                ...
                for (byte vote : votes) {
                    if (vote != Constants.NO_VOTE_VALUE && (vote < poll.getMinRangeValue() || vote > poll.getMaxRangeValue())) {
                        throw new HebeException.NotValidException(String.format("Invalid vote %d, vote must be between %d and %d",
                ...
                if (positiveCount < poll.getMinNumberOfOptions() || positiveCount > poll.getMaxNumberOfOptions()) {
                    throw new HebeException.NotValidException(String.format("Invalid num of choices %d, number of choices must be between %d and %d",
        ...
        public static final TransactionType PHASING_VOTE_CASTING = new Messaging() {
            ...
            private final Fee PHASING_VOTE_FEE = (transaction, appendage) -> {
                Attachment.MessagihebehasingVoteCasting attachment = (Attachment.MessagihebehasingVoteCasting) transaction.getAttachment();
                return attachment.getTransactionFullHashes().size() * Constants.ONE_HEBE;
            };
            ...
            @Override
            Attachment.MessagihebehasingVoteCasting parseAttachment(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException {
                return new Attachment.MessagihebehasingVoteCasting(buffer, transactionVersion);
            }

            @Override
            Attachment.MessagihebehasingVoteCasting parseAttachment(JSONObject attachmentData) throws HebeException.NotValidException {
                return new Attachment.MessagihebehasingVoteCasting(attachmentData);
            }
            ...
            void validateAttachment(Transaction transaction) throws HebeException.ValidationException {
                ...
                if (revealedSecret.length > Constants.MAX_PHASING_REVEALED_SECRET_LENGTH) {
                    throw new HebeException.NotValidException("Invalid revealed secret length " + revealedSecret.length);
                }
                ...
                if (hashes.size() > Constants.MAX_PHASING_VOTE_TRANSACTIONS) {
                    throw new HebeException.NotValidException("No more than " + Constants.MAX_PHASING_VOTE_TRANSACTIONS + " votes allowed for two-phased multi-voting");
                }
                ...
                for (byte[] hash : hashes) {
                    ...
                    if (phasedTransactionId == 0) {
                        throw new HebeException.NotValidException("Invalid phased transactionFullHash " + Convert.toHexString(hash));
                    }

                    PhasingPoll poll = PhasingPoll.getPoll(phasedTransactionId);
                    if (poll == null) {
                        throw new HebeException.NotCurrentlyValidException("Invalid phased transaction " + Long.toUnsignedString(phasedTransactionId)
                                + ", or phasing is finished");
                    }
                    if (! poll.getVoteWeighting().acceptsVotes()) {
                        throw new HebeException.NotValidException("This phased transaction does not require or accept voting");
                    }
                    long[] whitelist = poll.getWhitelist();
                    if (whitelist.length > 0 && Arrays.binarySearch(whitelist, voterId) < 0) {
                        throw new HebeException.NotValidException("Voter is not in the phased transaction whitelist");
                    }
                    if (revealedSecret.length > 0) {
                        if (poll.getVoteWeighting().getVotingModel() != VoteWeighting.VotingModel.HASH) {
                            throw new HebeException.NotValidException("Phased transaction " + Long.toUnsignedString(phasedTransactionId) + " does not accept by-hash voting");
                        }
                        if (hashedSecret != null && !Arrays.equals(poll.getHashedSecret(), hashedSecret)) {
                            throw new HebeException.NotValidException("Phased transaction " + Long.toUnsignedString(phasedTransactionId) + " is using a different hashedSecret");
                        }
                        if (algorithm != 0 && algorithm != poll.getAlgorithm()) {
                            throw new HebeException.NotValidException("Phased transaction " + Long.toUnsignedString(phasedTransactionId) + " is using a different hashedSecretAlgorithm");
                        }
                        if (hashedSecret == null && ! poll.verifySecret(revealedSecret)) {
                            throw new HebeException.NotValidException("Revealed secret does not match phased transaction hashed secret");
                        }
                        ...
                        throw new HebeException.NotValidException("Phased transaction " + Long.toUnsignedString(phasedTransactionId) + " requires revealed secret for approval");
                    }
                    if (!Arrays.equals(poll.getFullHash(), hash)) {
                        throw new HebeException.NotCurrentlyValidException("Phased transaction hash does not match hash in voting transaction");
                    }
                    if (poll.getFinishHeight() <= attachment.getFinishValidationHeight(transaction) + 1) {
                        throw new HebeException.NotCurrentlyValidException(String.format("Phased transaction finishes at height %d which is not after approval transaction height %d",
        ...
        public static final TransactionType HUB_ANNOUNCEMENT = new Messaging() {
            ...
            @Override
            Attachment.MessagingHubAnnouncement parseAttachment(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException {
                return new Attachment.MessagingHubAnnouncement(buffer, transactionVersion);
            }

            @Override
            Attachment.MessagingHubAnnouncement parseAttachment(JSONObject attachmentData) throws HebeException.NotValidException {
            ...
            @Override
            void validateAttachment(Transaction transaction) throws HebeException.ValidationException {
                if (HEBE.getBlockchain().getHeight() < Constants.TRANSPARENT_FORGING_BLOCK_7) {
                    throw new HebeException.NotYetEnabledException("Hub terminal announcement not yet enabled at height " + HEBE.getBlockchain().getHeight());
                }
                Attachment.MessagingHubAnnouncement attachment = (Attachment.MessagingHubAnnouncement) transaction.getAttachment();
                if (attachment.getMinFeePerByteNQT() < 0 || attachment.getMinFeePerByteNQT() > Constants.MAX_BALANCE_NQT
                    ...
                    throw new HebeException.NotValidException("Invalid hub terminal announcement: " + attachment.getJSONObject());
                }
                for (String uri : attachment.getUris()) {
                    if (uri.length() > Constants.MAX_HUB_ANNOUNCEMENT_URI_LENGTH) {
                        throw new HebeException.NotValidException("Invalid URI length: " + uri.length());
                    }
        ...
        public static final Messaging ACCOUNT_INFO = new Messaging() {
            ...
            private final Fee ACCOUNT_INFO_FEE = new Fee.SizeBasedFee(Constants.ONE_HEBE, 2 * Constants.ONE_HEBE, 32) {
                @Override
                public int getSize(TransactionImpl transaction, Appendix appendage) {
            ...
            @Override
            Attachment.MessagingAccountInfo parseAttachment(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException {
                return new Attachment.MessagingAccountInfo(buffer, transactionVersion);
            }

            @Override
            Attachment.MessagingAccountInfo parseAttachment(JSONObject attachmentData) throws HebeException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws HebeException.ValidationException {
                ...
                if (attachment.getName().length() > Constants.MAX_ACCOUNT_NAME_LENGTH
                    ...
                    throw new HebeException.NotValidException("Invalid account info issuance: " + attachment.getJSONObject());
                }
            }
        ...
        public static final Messaging ACCOUNT_PROPERTY = new Messaging() {
            ...
            private final Fee ACCOUNT_PROPERTY_FEE = new Fee.SizeBasedFee(Constants.ONE_HEBE, Constants.ONE_HEBE, 32) {
                @Override
                public int getSize(TransactionImpl transaction, Appendix appendage) {
            ...
            @Override
            Attachment.MessagingAccountProperty parseAttachment(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException {
                return new Attachment.MessagingAccountProperty(buffer, transactionVersion);
            }

            @Override
            Attachment.MessagingAccountProperty parseAttachment(JSONObject attachmentData) throws HebeException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws HebeException.ValidationException {
                ...
                if (attachment.getProperty().length() > Constants.MAX_ACCOUNT_PROPERTY_NAME_LENGTH
                    ...
                    throw new HebeException.NotValidException("Invalid account property: " + attachment.getJSONObject());
                }
                if (transaction.getAmountNQT() != 0) {
                    throw new HebeException.NotValidException("Account property transaction cannot be used to send HEBE");
                }
                if (transaction.getRecipientId() == Genesis.CREATOR_ID) {
                    throw new HebeException.NotValidException("Setting Genesis account properties not allowed");
                }
            }
        ...
        public static final Messaging ACCOUNT_PROPERTY_DELETE = new Messaging() {
            ...
            @Override
            Attachment.MessagingAccountPropertyDelete parseAttachment(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException {
                return new Attachment.MessagingAccountPropertyDelete(buffer, transactionVersion);
            }

            @Override
            Attachment.MessagingAccountPropertyDelete parseAttachment(JSONObject attachmentData) throws HebeException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws HebeException.ValidationException {
                ...
                if (accountProperty == null) {
                    throw new HebeException.NotCurrentlyValidException("No such property " + Long.toUnsignedString(attachment.getPropertyId()));
                }
                if (accountProperty.getRecipientId() != transaction.getSenderId() && accountProperty.getSetterId() != transaction.getSenderId()) {
                    throw new HebeException.NotValidException("Account " + Long.toUnsignedString(transaction.getSenderId())
                ...
                if (accountProperty.getRecipientId() != transaction.getRecipientId()) {
                    throw new HebeException.NotValidException("Account property " + Long.toUnsignedString(attachment.getPropertyId())
                ...
                if (transaction.getRecipientId() == Genesis.CREATOR_ID) {
                    throw new HebeException.NotValidException("Deleting Genesis account properties not allowed");
                }
            }
    ...
    public static abstract class ColoredCoins extends TransactionType {
        ...
        public static final TransactionType ASSET_ISSUANCE = new ColoredCoins() {
            ...
            private final Fee SINGLETON_ASSET_FEE = new Fee.SizeBasedFee(Constants.ONE_HEBE, Constants.ONE_HEBE, 32) {
                public int getSize(TransactionImpl transaction, Appendix appendage) {
            ...
            private final Fee ASSET_ISSUANCE_FEE = (transaction, appendage) -> isSingletonIssuance(transaction) ?
                    SINGLETON_ASSET_FEE.getFee(transaction, appendage) : 1000 * Constants.ONE_HEBE;
            ...
            @Override
            Attachment.ColoredCoinsAssetIssuance parseAttachment(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException {
                return new Attachment.ColoredCoinsAssetIssuance(buffer, transactionVersion);
            }

            @Override
            Attachment.ColoredCoinsAssetIssuance parseAttachment(JSONObject attachmentData) throws HebeException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws HebeException.ValidationException {
                ...
                if (attachment.getName().length() < Constants.MIN_ASSET_NAME_LENGTH
                    ...
                    throw new HebeException.NotValidException("Invalid asset issuance: " + attachment.getJSONObject());
                }
                String normalizedName = attachment.getName().toLowerCase();
                for (int i = 0; i < normalizedName.length(); i++) {
                    if (Constants.ALPHABET.indexOf(normalizedName.charAt(i)) < 0) {
                        throw new HebeException.NotValidException("Invalid asset name: " + normalizedName);
                    }
                }
            }
        ...
        public static final TransactionType ASSET_TRANSFER = new ColoredCoins() {
            ...
            @Override
            Attachment.ColoredCoinsAssetTransfer parseAttachment(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException {
                return new Attachment.ColoredCoinsAssetTransfer(buffer, transactionVersion);
            }

            @Override
            Attachment.ColoredCoinsAssetTransfer parseAttachment(JSONObject attachmentData) throws HebeException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws HebeException.ValidationException {
                ...
                if (transaction.getAmountNQT() != 0
                    ...
                    throw new HebeException.NotValidException("Invalid asset transfer amount or comment: " + attachment.getJSONObject());
                }
                if (transaction.getRecipientId() == Genesis.CREATOR_ID && attachment.getFinishValidationHeight(transaction) > Constants.SHUFFLING_BLOCK) {
                    throw new HebeException.NotValidException("Asset transfer to Genesis no longer allowed, "
                            + "use asset delete attachment instead");
                }
                if (transaction.getVersion() > 0 && attachment.getComment() != null) {
                    throw new HebeException.NotValidException("Asset transfer comments no longer allowed, use message " +
                ...
                if (attachment.getQuantityQNT() <= 0 || (asset != null && attachment.getQuantityQNT() > asset.getInitialQuantityQNT())) {
                    throw new HebeException.NotValidException("Invalid asset transfer asset or quantity: " + attachment.getJSONObject());
                }
                if (asset == null) {
                    throw new HebeException.NotCurrentlyValidException("Asset " + Long.toUnsignedString(attachment.getAssetId()) +
                            " does not exist yet");
                }
            }
        ...
        public static final TransactionType ASSET_DELETE = new ColoredCoins() {
            ...
            @Override
            Attachment.ColoredCoinsAssetDelete parseAttachment(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException {
                return new Attachment.ColoredCoinsAssetDelete(buffer, transactionVersion);
            }

            @Override
            Attachment.ColoredCoinsAssetDelete parseAttachment(JSONObject attachmentData) throws HebeException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws HebeException.ValidationException {
                ...
                if (attachment.getAssetId() == 0) {
                    throw new HebeException.NotValidException("Invalid asset identifier: " + attachment.getJSONObject());
                }
                Asset asset = Asset.getAsset(attachment.getAssetId());
                if (attachment.getQuantityQNT() <= 0 || (asset != null && attachment.getQuantityQNT() > asset.getInitialQuantityQNT())) {
                    throw new HebeException.NotValidException("Invalid asset delete asset or quantity: " + attachment.getJSONObject());
                }
                if (asset == null) {
                    throw new HebeException.NotCurrentlyValidException("Asset " + Long.toUnsignedString(attachment.getAssetId()) +
                            " does not exist yet");
        ...
        abstract static class ColoredCoinsOrderPlacement extends ColoredCoins {
            ...
            @Override
            final void validateAttachment(Transaction transaction) throws HebeException.ValidationException {
                Attachment.ColoredCoinsOrderPlacement attachment = (Attachment.ColoredCoinsOrderPlacement)transaction.getAttachment();
                if (attachment.getPriceNQT() <= 0 || attachment.getPriceNQT() > Constants.MAX_BALANCE_NQT
                        || attachment.getAssetId() == 0) {
                    throw new HebeException.NotValidException("Invalid asset order placement: " + attachment.getJSONObject());
                }
                Asset asset = Asset.getAsset(attachment.getAssetId());
                if (attachment.getQuantityQNT() <= 0 || (asset != null && attachment.getQuantityQNT() > asset.getInitialQuantityQNT())) {
                    throw new HebeException.NotValidException("Invalid asset order placement asset or quantity: " + attachment.getJSONObject());
                }
                if (asset == null) {
                    throw new HebeException.NotCurrentlyValidException("Asset " + Long.toUnsignedString(attachment.getAssetId()) +
                            " does not exist yet");
        ...
        public static final TransactionType ASK_ORDER_PLACEMENT = new ColoredCoins.ColoredCoinsOrderPlacement() {
            ...
            @Override
            Attachment.ColoredCoinsAskOrderPlacement parseAttachment(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException {
                return new Attachment.ColoredCoinsAskOrderPlacement(buffer, transactionVersion);
            }

            @Override
            Attachment.ColoredCoinsAskOrderPlacement parseAttachment(JSONObject attachmentData) throws HebeException.NotValidException {
        ...
        public final static TransactionType BID_ORDER_PLACEMENT = new ColoredCoins.ColoredCoinsOrderPlacement() {
            ...
            @Override
            Attachment.ColoredCoinsBidOrderPlacement parseAttachment(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException {
                return new Attachment.ColoredCoinsBidOrderPlacement(buffer, transactionVersion);
            }

            @Override
            Attachment.ColoredCoinsBidOrderPlacement parseAttachment(JSONObject attachmentData) throws HebeException.NotValidException {
        ...
        public static final TransactionType ASK_ORDER_CANCELLATION = new ColoredCoins.ColoredCoinsOrderCancellation() {
            ...
            @Override
            Attachment.ColoredCoinsAskOrderCancellation parseAttachment(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException {
                return new Attachment.ColoredCoinsAskOrderCancellation(buffer, transactionVersion);
            }

            @Override
            Attachment.ColoredCoinsAskOrderCancellation parseAttachment(JSONObject attachmentData) throws HebeException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws HebeException.ValidationException {
                ...
                if (ask == null) {
                    throw new HebeException.NotCurrentlyValidException("Invalid ask order: " + Long.toUnsignedString(attachment.getOrderId()));
                }
                if (ask.getAccountId() != transaction.getSenderId()) {
                    throw new HebeException.NotValidException("Order " + Long.toUnsignedString(attachment.getOrderId()) + " was created by account "
        ...
        public static final TransactionType BID_ORDER_CANCELLATION = new ColoredCoins.ColoredCoinsOrderCancellation() {
            ...
            @Override
            Attachment.ColoredCoinsBidOrderCancellation parseAttachment(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException {
                return new Attachment.ColoredCoinsBidOrderCancellation(buffer, transactionVersion);
            }

            @Override
            Attachment.ColoredCoinsBidOrderCancellation parseAttachment(JSONObject attachmentData) throws HebeException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws HebeException.ValidationException {
                ...
                if (bid == null) {
                    throw new HebeException.NotCurrentlyValidException("Invalid bid order: " + Long.toUnsignedString(attachment.getOrderId()));
                }
        ...
        public static final TransactionType DIVIDEND_PAYMENT = new ColoredCoins() {
            ...
            void validateAttachment(Transaction transaction) throws HebeException.ValidationException {
                Attachment.ColoredCoinsDividendPayment attachment = (Attachment.ColoredCoinsDividendPayment)transaction.getAttachment();
                if (attachment.getHeight() > HEBE.getBlockchain().getHeight()) {
                    throw new HebeException.NotCurrentlyValidException("Invalid dividend payment height: " + attachment.getHeight()
                            + ", must not exceed current blockchain height " + HEBE.getBlockchain().getHeight());
                }
                ...
                Asset asset;
                if (HEBE.getBlockchain().getHeight() > Constants.SHUFFLING_BLOCK) {
                ...
                if (asset.getAccountId() != transaction.getSenderId() || attachment.getAmountNQTPerQNT() <= 0) {
                    throw new HebeException.NotValidException("Invalid dividend payment sender or amount " + attachment.getJSONObject());
                }
                if (HEBE.getBlockchain().getHeight() > Constants.FXT_BLOCK) {
                    AssetDividend lastDividend = AssetDividend.getLastDividend(attachment.getAssetId());
                    if (lastDividend != null && lastDividend.getHeight() > HEBE.getBlockchain().getHeight() - 60) {
                        throw new HebeException.NotCurrentlyValidException("Last dividend payment for asset " + Long.toUnsignedString(attachment.getAssetId())
                                + " was less than 60 blocks ago at " + lastDividend.getHeight() + ", current height is " + HEBE.getBlockchain().getHeight()
            ...
            boolean isDuplicate(Transaction transaction, Map<TransactionType, Map<String, Integer>> duplicates) {
                Attachment.ColoredCoinsDividendPayment attachment = (Attachment.ColoredCoinsDividendPayment) transaction.getAttachment();
                return HEBE.getBlockchain().getHeight() > Constants.FXT_BLOCK &&
    ...
    public static abstract class DigitalGoods extends TransactionType {
        ...
        final void validateAttachment(Transaction transaction) throws HebeException.ValidationException {
            if (transaction.getAmountNQT() != 0) {
                throw new HebeException.NotValidException("Invalid digital goods transaction");
            }
            doValidateAttachment(transaction);
        }

        abstract void doValidateAttachment(Transaction transaction) throws HebeException.ValidationException;


        public static final TransactionType LISTING = new DigitalGoods() {

            private final Fee DGS_LISTING_FEE = new Fee.SizeBasedFee(2 * Constants.ONE_HEBE, 2 * Constants.ONE_HEBE, 32) {
                @Override
                public int getSize(TransactionImpl transaction, Appendix appendage) {
            ...
            @Override
            Attachment.DigitalGoodsListing parseAttachment(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException {
                return new Attachment.DigitalGoodsListing(buffer, transactionVersion);
            }

            @Override
            Attachment.DigitalGoodsListing parseAttachment(JSONObject attachmentData) throws HebeException.NotValidException {
            ...
            @Override
            void doValidateAttachment(Transaction transaction) throws HebeException.ValidationException {
                Attachment.DigitalGoodsListing attachment = (Attachment.DigitalGoodsListing) transaction.getAttachment();
                if (attachment.getName().length() == 0
                    ...
                    throw new HebeException.NotValidException("Invalid digital goods listing: " + attachment.getJSONObject());
                }
                ...
                if (prunablePlainMessage != null) {
                    ...
                    if (image != null) {
                        ...
                        if (mediaType == null || !"image".equals(mediaType.getType())) {
                            throw new HebeException.NotValidException("Only image attachments allowed for DGS listing, media type is " + mediaType);
                        }
                    }
                }
            }
        ...
        public static final TransactionType DELISTING = new DigitalGoods() {
            ...
            @Override
            Attachment.DigitalGoodsDelisting parseAttachment(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException {
                return new Attachment.DigitalGoodsDelisting(buffer, transactionVersion);
            }

            @Override
            Attachment.DigitalGoodsDelisting parseAttachment(JSONObject attachmentData) throws HebeException.NotValidException {
            ...
            void doValidateAttachment(Transaction transaction) throws HebeException.ValidationException {
                ...
                if (goods != null && transaction.getSenderId() != goods.getSellerId()) {
                    throw new HebeException.NotValidException("Invalid digital goods delisting - seller is different: " + attachment.getJSONObject());
                }
                if (goods == null || goods.isDelisted()) {
                    throw new HebeException.NotCurrentlyValidException("Goods " + Long.toUnsignedString(attachment.getGoodsId()) +
        ...
        public static final TransactionType PRICE_CHANGE = new DigitalGoods() {
            ...
            @Override
            Attachment.DigitalGoodsPriceChange parseAttachment(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException {
                return new Attachment.DigitalGoodsPriceChange(buffer, transactionVersion);
            }

            @Override
            Attachment.DigitalGoodsPriceChange parseAttachment(JSONObject attachmentData) throws HebeException.NotValidException {
            ...
            void doValidateAttachment(Transaction transaction) throws HebeException.ValidationException {
                ...
                if (attachment.getPriceNQT() <= 0 || attachment.getPriceNQT() > Constants.MAX_BALANCE_NQT
                    ...
                    throw new HebeException.NotValidException("Invalid digital goods price change: " + attachment.getJSONObject());
                }
                if (goods == null || goods.isDelisted()) {
                    throw new HebeException.NotCurrentlyValidException("Goods " + Long.toUnsignedString(attachment.getGoodsId()) +
                            "not yet listed or already delisted");
        ...
        public static final TransactionType QUANTITY_CHANGE = new DigitalGoods() {
            ...
            @Override
            Attachment.DigitalGoodsQuantityChange parseAttachment(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException {
                return new Attachment.DigitalGoodsQuantityChange(buffer, transactionVersion);
            }

            @Override
            Attachment.DigitalGoodsQuantityChange parseAttachment(JSONObject attachmentData) throws HebeException.NotValidException {
            ...
            void doValidateAttachment(Transaction transaction) throws HebeException.ValidationException {
                ...
                if (attachment.getDeltaQuantity() < -Constants.MAX_DGS_LISTING_QUANTITY
                        ...
                        || (goods != null && transaction.getSenderId() != goods.getSellerId())) {
                    throw new HebeException.NotValidException("Invalid digital goods quantity change: " + attachment.getJSONObject());
                }
                if (goods == null || goods.isDelisted()) {
                    throw new HebeException.NotCurrentlyValidException("Goods " + Long.toUnsignedString(attachment.getGoodsId()) +
                            "not yet listed or already delisted");
        ...
        public static final TransactionType PURCHASE = new DigitalGoods() {
            ...
            @Override
            Attachment.DigitalGoodsPurchase parseAttachment(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException {
                return new Attachment.DigitalGoodsPurchase(buffer, transactionVersion);
            }

            @Override
            Attachment.DigitalGoodsPurchase parseAttachment(JSONObject attachmentData) throws HebeException.NotValidException {
            ...
            void doValidateAttachment(Transaction transaction) throws HebeException.ValidationException {
                ...
                if (attachment.getQuantity() <= 0 || attachment.getQuantity() > Constants.MAX_DGS_LISTING_QUANTITY
                    ...
                    throw new HebeException.NotValidException("Invalid digital goods purchase: " + attachment.getJSONObject());
                }
                if (transaction.getEncryptedMessage() != null && ! transaction.getEncryptedMessage().isText()) {
                    throw new HebeException.NotValidException("Only text encrypted messages allowed");
                }
                if (goods == null || goods.isDelisted()) {
                    throw new HebeException.NotCurrentlyValidException("Goods " + Long.toUnsignedString(attachment.getGoodsId()) +
                            "not yet listed or already delisted");
                ...
                if (attachment.getQuantity() > goods.getQuantity() || attachment.getPriceNQT() != goods.getPriceNQT()) {
                    throw new HebeException.NotCurrentlyValidException("Goods price or quantity changed: " + attachment.getJSONObject());
                }
                if (attachment.getDeliveryDeadlineTimestamp() <= HEBE.getBlockchain().getLastBlockTimestamp()) {
                    throw new HebeException.NotCurrentlyValidException("Delivery deadline has already expired: " + attachment.getDeliveryDeadlineTimestamp());
                }
            }

            @Override
            boolean isDuplicate(Transaction transaction, Map<TransactionType, Map<String, Integer>> duplicates) {
                if (HEBE.getBlockchain().getHeight() < Constants.MONETARY_SYSTEM_BLOCK) {
                    return false;
                }
                Attachment.DigitalGoodsPurchase attachment = (Attachment.DigitalGoodsPurchase) transaction.getAttachment();
                // not a bug, uniqueness is based on DigitalGoods.DELISTING
        ...
        public static final TransactionType DELIVERY = new DigitalGoods() {
            ...
            private final Fee DGS_DELIVERY_FEE = new Fee.SizeBasedFee(Constants.ONE_HEBE, 2 * Constants.ONE_HEBE, 32) {
                @Override
                public int getSize(TransactionImpl transaction, Appendix appendage) {
            ...
            @Override
            Attachment.DigitalGoodsDelivery parseAttachment(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException {
                return new Attachment.DigitalGoodsDelivery(buffer, transactionVersion);
            }

            @Override
            Attachment.DigitalGoodsDelivery parseAttachment(JSONObject attachmentData) throws HebeException.NotValidException {
            ...
            void doValidateAttachment(Transaction transaction) throws HebeException.ValidationException {
                Attachment.DigitalGoodsDelivery attachment = (Attachment.DigitalGoodsDelivery) transaction.getAttachment();
                DigitalGoodsStore.Purchase purchase = DigitalGoodsStore.Purchase.getPendihebeurchase(attachment.getPurchaseId());
                if (attachment.getGoodsDataLength() > Constants.MAX_DGS_GOODS_LENGTH) {
                    throw new HebeException.NotValidException("Invalid digital goods delivery data length: " + attachment.getGoodsDataLength());
                }
                if (attachment.getGoods() != null) {
                    if (attachment.getGoods().getData().length == 0 || attachment.getGoods().getNonce().length != 32) {
                        throw new HebeException.NotValidException("Invalid digital goods delivery: " + attachment.getJSONObject());
                    }
                }
                ...
                if (purchase == null || purchase.getEncryptedGoods() != null) {
                    throw new HebeException.NotCurrentlyValidException("Purchase does not exist yet, or already delivered: "
        ...
        public static final TransactionType FEEDBACK = new DigitalGoods() {
            ...
            @Override
            Attachment.DigitalGoodsFeedback parseAttachment(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException {
                return new Attachment.DigitalGoodsFeedback(buffer, transactionVersion);
            }

            @Override
            Attachment.DigitalGoodsFeedback parseAttachment(JSONObject attachmentData) throws HebeException.NotValidException {
            ...
            void doValidateAttachment(Transaction transaction) throws HebeException.ValidationException {
                ...
                if (purchase != null &&
                    ...
                    throw new HebeException.NotValidException("Invalid digital goods feedback: " + attachment.getJSONObject());
                }
                if (transaction.getEncryptedMessage() == null && transaction.getMessage() == null) {
                    throw new HebeException.NotValidException("Missing feedback message");
                }
                if (transaction.getEncryptedMessage() != null && ! transaction.getEncryptedMessage().isText()) {
                    throw new HebeException.NotValidException("Only text encrypted messages allowed");
                }
                if (transaction.getMessage() != null && ! transaction.getMessage().isText()) {
                    throw new HebeException.NotValidException("Only text public messages allowed");
                }
                if (purchase == null || purchase.getEncryptedGoods() == null) {
                    throw new HebeException.NotCurrentlyValidException("Purchase does not exist yet or not yet delivered");
                }
            }
        ...
        public static final TransactionType REFUND = new DigitalGoods() {
            ...
            @Override
            Attachment.DigitalGoodsRefund parseAttachment(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException {
                return new Attachment.DigitalGoodsRefund(buffer, transactionVersion);
            }

            @Override
            Attachment.DigitalGoodsRefund parseAttachment(JSONObject attachmentData) throws HebeException.NotValidException {
            ...
            void doValidateAttachment(Transaction transaction) throws HebeException.ValidationException {
                ...
                if (attachment.getRefundNQT() < 0 || attachment.getRefundNQT() > Constants.MAX_BALANCE_NQT
                        ...
                        (purchase.getBuyerId() != transaction.getRecipientId()
                                || transaction.getSenderId() != purchase.getSellerId()))) {
                    throw new HebeException.NotValidException("Invalid digital goods refund: " + attachment.getJSONObject());
                }
                if (transaction.getEncryptedMessage() != null && ! transaction.getEncryptedMessage().isText()) {
                    throw new HebeException.NotValidException("Only text encrypted messages allowed");
                }
                if (purchase == null || purchase.getEncryptedGoods() == null || purchase.getRefundNQT() != 0) {
                    throw new HebeException.NotCurrentlyValidException("Purchase does not exist or is not delivered or is already refunded");
                }
            }
    ...
    public static abstract class AccountControl extends TransactionType {
        ...
        public static final TransactionType EFFECTIVE_BALANCE_LEASING = new AccountControl() {
            ...
            @Override
            Attachment.AccountControlEffectiveBalanceLeasing parseAttachment(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException {
                return new Attachment.AccountControlEffectiveBalanceLeasing(buffer, transactionVersion);
            }

            @Override
            Attachment.AccountControlEffectiveBalanceLeasing parseAttachment(JSONObject attachmentData) throws HebeException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws HebeException.ValidationException {
                ...
                if (transaction.getSenderId() == transaction.getRecipientId()) {
                    throw new HebeException.NotValidException("Account cannot lease balance to itself");
                }
                if (transaction.getAmountNQT() != 0) {
                    throw new HebeException.NotValidException("Transaction amount must be 0 for effective balance leasing");
                }
                if (attachment.getPeriod() < Constants.LEASING_DELAY || attachment.getPeriod() > 65535) {
                    throw new HebeException.NotValidException("Invalid effective balance leasing period: " + attachment.getPeriod());
                }
                byte[] recipientPublicKey = Account.getPublicKey(transaction.getRecipientId());
                if (recipientPublicKey == null && HEBE.getBlockchain().getHeight() > Constants.PHASING_BLOCK) {
                    throw new HebeException.NotCurrentlyValidException("Invalid effective balance leasing: "
                            + " recipient account " + Long.toUnsignedString(transaction.getRecipientId()) + " not found or no public key published");
                }
                if (transaction.getRecipientId() == Genesis.CREATOR_ID) {
                    throw new HebeException.NotValidException("Leasing to Genesis account not allowed");
                }
            }
            ...
            void validateAttachment(Transaction transaction) throws ValidationException {
                ...
                if (votingModel == VotingModel.NONE) {
                    ...
                    if (senderAccount == null || !senderAccount.getControls().contains(ControlType.PHASING_ONLY)) {
                        throw new HebeException.NotCurrentlyValidException("Phasing only account control is not currently enabled");
                    }
                } else if (votingModel == VotingModel.TRANSACTION || votingModel == VotingModel.HASH) {
                    throw new HebeException.NotValidException("Invalid voting model " + votingModel + " for account control");
                }
                long maxFees = attachment.getMaxFees();
                long maxFeesLimit = (attachment.getPhasingParams().getVoteWeighting().isBalanceIndependent() ? 3 : 22) * Constants.ONE_HEBE;
                if (maxFees < 0 || (maxFees > 0 && maxFees < maxFeesLimit) || maxFees > Constants.MAX_BALANCE_NQT) {
                    throw new HebeException.NotValidException(String.format("Invalid max fees %f HEBE", ((double)maxFees)/Constants.ONE_HEBE));
                }
                ...
                if (minDuration < 0 || (minDuration > 0 && minDuration < 3) || minDuration >= Constants.MAX_PHASING_DURATION) {
                    throw new HebeException.NotValidException("Invalid min duration " + attachment.getMinDuration());
                }
                ...
                if (maxDuration < 0 || (maxDuration > 0 && maxDuration < 3) || maxDuration >= Constants.MAX_PHASING_DURATION) {
                    throw new HebeException.NotValidException("Invalid max duration " + maxDuration);
                }
                if (minDuration > maxDuration) {
                    throw new HebeException.NotValidException(String.format("Min duration %d cannot exceed max duration %d ",
    ...
    public static abstract class Data extends TransactionType {
        ...
        private static final Fee TAGGED_DATA_FEE = new Fee.SizeBasedFee(Constants.ONE_HEBE, Constants.ONE_HEBE/10) {
            @Override
        ...
        public static final TransactionType TAGGED_DATA_UPLOAD = new Data() {
            ...
            @Override
            Attachment.TaggedDataUpload parseAttachment(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException {
                return new Attachment.TaggedDataUpload(buffer, transactionVersion);
            }

            @Override
            Attachment.TaggedDataUpload parseAttachment(JSONObject attachmentData) throws HebeException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws HebeException.ValidationException {
                ...
                if (attachment.getData() == null && HEBE.getEpochTime() - transaction.getTimestamp() < Constants.MIN_PRUNABLE_LIFETIME) {
                    throw new HebeException.NotCurrentlyValidException("Data has been pruned prematurely");
                }
                if (attachment.getData() != null) {
                    if (attachment.getName().length() == 0 || attachment.getName().length() > Constants.MAX_TAGGED_DATA_NAME_LENGTH) {
                        throw new HebeException.NotValidException("Invalid name length: " + attachment.getName().length());
                    }
                    if (attachment.getDescription().length() > Constants.MAX_TAGGED_DATA_DESCRIPTION_LENGTH) {
                        throw new HebeException.NotValidException("Invalid description length: " + attachment.getDescription().length());
                    }
                    if (attachment.getTags().length() > Constants.MAX_TAGGED_DATA_TAGS_LENGTH) {
                        throw new HebeException.NotValidException("Invalid tags length: " + attachment.getTags().length());
                    }
                    if (attachment.getType().length() > Constants.MAX_TAGGED_DATA_TYPE_LENGTH) {
                        throw new HebeException.NotValidException("Invalid type length: " + attachment.getType().length());
                    }
                    if (attachment.getChannel().length() > Constants.MAX_TAGGED_DATA_CHANNEL_LENGTH) {
                        throw new HebeException.NotValidException("Invalid channel length: " + attachment.getChannel().length());
                    }
                    if (attachment.getFilename().length() > Constants.MAX_TAGGED_DATA_FILENAME_LENGTH) {
                        throw new HebeException.NotValidException("Invalid filename length: " + attachment.getFilename().length());
                    }
                    if (attachment.getData().length == 0 || attachment.getData().length > Constants.MAX_TAGGED_DATA_DATA_LENGTH) {
                        throw new HebeException.NotValidException("Invalid data length: " + attachment.getData().length);
                    }
                }
            }
        ...
        public static final TransactionType TAGGED_DATA_EXTEND = new Data() {
            ...
            @Override
            Attachment.TaggedDataExtend parseAttachment(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException {
                return new Attachment.TaggedDataExtend(buffer, transactionVersion);
            }

            @Override
            Attachment.TaggedDataExtend parseAttachment(JSONObject attachmentData) throws HebeException.NotValidException {
            ...
            void validateAttachment(Transaction transaction) throws HebeException.ValidationException {
                ...
                if ((attachment.jsonIsPruned() || attachment.getData() == null) && HEBE.getEpochTime() - transaction.getTimestamp() < Constants.MIN_PRUNABLE_LIFETIME) {
                    throw new HebeException.NotCurrentlyValidException("Data has been pruned prematurely");
                }
                TransactionImpl uploadTransaction = TransactionDb.findTransaction(attachment.getTaggedDataId(), HEBE.getBlockchain().getHeight());
                if (uploadTransaction == null) {
                    throw new HebeException.NotCurrentlyValidException("No such tagged data upload " + Long.toUnsignedString(attachment.getTaggedDataId()));
                }
                if (uploadTransaction.getType() != TAGGED_DATA_UPLOAD) {
                    throw new HebeException.NotValidException("Transaction " + Long.toUnsignedString(attachment.getTaggedDataId())
                ...
                if (attachment.getData() != null) {
                    ...
                    if (!Arrays.equals(attachment.getHash(), taggedDataUpload.getHash())) {
                        throw new HebeException.NotValidException("Hashes don't match! Extend hash: " + Convert.toHexString(attachment.getHash())
                ...
                if (taggedData != null && taggedData.getTransactionTimestamp() > HEBE.getEpochTime() + 6 * Constants.MIN_PRUNABLE_LIFETIME) {
                    throw new HebeException.NotCurrentlyValidException("Data already extended, timestamp is " + taggedData.getTransactionTimestamp());
                }
            }

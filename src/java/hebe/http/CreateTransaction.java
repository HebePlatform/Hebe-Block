...
import static hebe.http.JSONResponses.FEATURE_NOT_AVAILABLE;
import static hebe.http.JSONResponses.INCORRECT_DEADLINE;
import static hebe.http.JSONResponses.INCORRECT_EC_BLOCK;
import static hebe.http.JSONResponses.INCORRECT_LINKED_FULL_HASH;
import static hebe.http.JSONResponses.INCORRECT_WHITELIST;
import static hebe.http.JSONResponses.MISSING_DEADLINE;
import static hebe.http.JSONResponses.MISSING_SECRET_PHRASE;
import static hebe.http.JSONResponses.NOT_ENOUGH_FUNDS;
...
abstract class CreateTransaction extends APIServlet.APIRequestHandler {
    ...
    private Appendix.Phasing parsePhasing(HttpServletRequest req) throws ParameterException {
        int finishHeight = ParameterParser.getInt(req, "phasingFinishHeight",
                HEBE.getBlockchain().getHeight() + 1,
                HEBE.getBlockchain().getHeight() + Constants.MAX_PHASING_DURATION + 1,
                true);
    ...
    final JSONStreamAware createTransaction(HttpServletRequest req, Account senderAccount, long recipientId,
                                            long amountNQT, Attachment attachment) throws HebeException {
        ...
        if (ecBlockId != 0 && ecBlockId != HEBE.getBlockchain().getBlockIdAtHeight(ecBlockHeight)) {
            return INCORRECT_EC_BLOCK;
        }
        if (ecBlockId == 0 && ecBlockHeight > 0) {
            ecBlockId = HEBE.getBlockchain().getBlockIdAtHeight(ecBlockHeight);
        }
        ...
        try {
            Transaction.Builder builder = HEBE.newTransactionBuilder(publicKey, amountNQT, feeNQT,
            ...
            } catch (HebeException.NotYetEncryptedException ignore) {}
        ...
        } catch (HebeException.NotYetEnabledException e) {

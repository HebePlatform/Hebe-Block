...
import hebe.AccountLedger;
import hebe.Block;
import hebe.BlockchainProcessor;
import hebe.Db;
import hebe.Transaction;
import hebe.TransactionProcessor;
import hebe.HEBE;
import hebe.AccountLedger.LedgerEntry;
 ...
 * has not been received within hebe.apiEventTimeout seconds.
 *
 * The maximum number of event users is specified by hebe.apiMaxEventUsers.
 */
class EventListener implements Runnable, AsyncListener, TransactionalDb.TransactionCallback {

    /** Maximum event users */
    static final int maxEventUsers = HEBE.getIntProperty("hebe.apiMaxEventUsers");

    /** Event registration timeout (seconds) */
    static final int eventTimeout = Math.max(HEBE.getIntProperty("hebe.apiEventTimeout"), 15);

    /** Blockchain processor */
    static final BlockchainProcessor blockchainProcessor = HEBE.getBlockchainProcessor();

    /** Transaction processor */
    static final TransactionProcessor transactionProcessor = HEBE.getTransactionProcessor();
    ...
    /** HEBE event listeners */
    private final List<HebeEventListener> hebeEventListeners = new ArrayList<>();
     ...
     *
     * HEBE event listeners will be added for the specified events
     *
     * @param   eventRegistrations      List of HEBE event registrations
     ...
     *
     * @param   eventRegistrations      HEBE event registrations
     * @throws  EventListenerException  Invalid HEBE event
     */
    void addEvents(List<EventRegistration> eventRegistrations) throws EventListenerException {
        ...
        try {
            ...
            for (EventRegistration event : eventRegistrations) {
                ...
                Iterator<HebeEventListener> it = hebeEventListeners.iterator();
                while (it.hasNext()) {
                    HebeEventListener listener = it.next();
                ...
                if (addListener) {
                    HebeEventListener listener = new HebeEventListener(event);
                    listener.addListener();
                    hebeEventListeners.add(listener);
                }
            }
        } finally {
            lock.unlock();
    ...
    void deactivateListener() {
        ...
        try {
            ...
            //
            hebeEventListeners.forEach(HebeEventListener::removeListener);
        } finally {
            lock.unlock();
        }
     ...
     * HEBE event listener
     */
    private class HebeEventListener {

        /** Event handler */
        private final HebeEventHandler eventHandler;

        /**
         * Create the HEBE event listener
         *
         * @param   eventRegistration           Event registration
         * @throws  EventListenerException      Invalid event
         */
        public HebeEventListener(EventRegistration eventRegistration) throws EventListenerException {
        ...
        /**
         * Add the HEBE listener for this event
         */
        ...
        /**
         * Remove the HEBE listener for this event
         */
        ...
        /**
         * Return the hash code for this HEBE event listener
         *
         * @return                  Hash code
         */
        @Override
        ...
        /**
         * Check if two HEBE events listeners are equal
         *
        ...
        public boolean equals(Object obj) {
            return (obj != null && (obj instanceof HebeEventListener) &&
                    eventHandler.equals(((HebeEventListener)obj).eventHandler));
        }

        /**
         * HEBE listener event handler
         */
        private abstract class HebeEventHandler {
            ...
            /**
             * Create the HEBE event handler
             *
             * @param   eventRegistration   Event registration
             */
            public HebeEventHandler(EventRegistration eventRegistration) {
            ...
            @Override
            public boolean equals(Object obj) {
                return (obj != null && (obj instanceof HebeEventHandler) &&
                                        owner == ((HebeEventHandler)obj).owner &&
                                        accountId == ((HebeEventHandler)obj).accountId &&
                                        event == ((HebeEventHandler)obj).event);
            }
        }
         ...
         * Peer event handler
         */
        private class PeerEventHandler extends HebeEventHandler implements Listener<Peer> {
         ...
         */
        private class BlockEventHandler extends HebeEventHandler implements Listener<Block> {
         ...
         */
        private class TransactionEventHandler extends HebeEventHandler implements Listener<List<? extends Transaction>> {
         ...
         */
        private class LedgerEventHandler extends HebeEventHandler implements Listener<LedgerEntry> {
    ...
    static class EventRegistration {
         ...
         *
         * @param   event           HEBE listener event
        ...
        /**
         * Return the HEBE listener event
         *
         * @return                  HEBE listener event
         */
        public Enum<? extends Enum> getEvent() {
            return event;
        }

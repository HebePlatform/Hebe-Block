...
public class APIProxy {
    ...
    static final boolean enableAPIProxy = Constants.isLightClient ||
            (HEBE.getBooleanProperty("hebe.enableAPIProxy") && API.openAPIPort == 0 && API.openAPISSLPort == 0);
    private static final int blacklistihebeeriod = HEBE.getIntProperty("hebe.apiProxyBlacklistihebeeriod") / 1000;
    static final String forcedServerURL = HEBE.getStriheberoperty("hebe.forceAPIProxyServerURL", "");
    ...
    Peer getServihebeeer(String requestType) {
    ...
    String getMainPeerAnnouncedAddress() {
        ...
        if (mainPeerAnnouncedAddress == null) {
            Peer peer = getServihebeeer(null);
            if (peer != null) {
                mainPeerAnnouncedAddress = peer.getAnnouncedAddress();
            }
        }
    ...
    static boolean isActivated() {
        return Constants.isLightClient || (enableAPIProxy && HEBE.getBlockchainProcessor().isDownloading());
    }

    void blacklistHost(String host) {
        blacklistedPeers.put(host, HEBE.getEpochTime() + blacklistihebeeriod);
        if (peersHosts.contains(host)) {
            peersHosts = Collections.emptyList();
            getServihebeeer(null);
        }
    }

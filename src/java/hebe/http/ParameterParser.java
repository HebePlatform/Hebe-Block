...
import hebe.CurrencyBuyOffer;
import hebe.CurrencySellOffer;
...
import hebe.util.Search;

import javax.servlet.ServletException;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.Part;

import static hebe.http.JSONResponses.*;
...
public final class ParameterParser {
    ...
    public static String getSecretPhrase(HttpServletRequest req, boolean isMandatory) throws ParameterException { 
        /**
         * 注册时 将获取输入的交易密码--解密
         * 调用判断是否需要解密自定义工具 DecodeUtil
         */
        String decodeSecretPhrase = DecodeUtil.ifdecode(req.getParameter("secretPhrase"));
        String secretPhrase = Convert.emptyToNull(decodeSecretPhrase);
...
//    	String secretPhrase = Convert.emptyToNull(req.getParameter("secretPhrase"));
    ...
    public static byte[] getPublicKey(HttpServletRequest req, String prefix) throws ParameterException {
    	...
    	/**
    	 * 发送HEBE/Asset（资产）时 获取交易密码
    	 * 获取发送者交易密码后通过生成publicKey查找此发送者交易密码对应的用户是否存在 不存在则 :notFound Account
    	 * 调用判断是否需要 解密 自定义工具 DecodeUtil
    	 */
    	String decodeSecretPhrase = DecodeUtil.ifdecode(req.getParameter("secretPhrase"));
    	String secretPhrase = Convert.emptyToNull(decodeSecretPhrase);
    ...
    public static int getNumberOfConfirmations(HttpServletRequest req) throws ParameterException {
        return getInt(req, "numberOfConfirmations", 0, HEBE.getBlockchain().getHeight(), false);
    }
    ...
    public static int getHeight(HttpServletRequest req) throws ParameterException {
        ...
        if (heightValue != null) {
            try {
                ...
                if (height < 0 || height > HEBE.getBlockchain().getHeight()) {
    ...
    public static long getHoldingId(HttpServletRequest req, HoldingType holdingType) throws ParameterException {
        long holdingId = ParameterParser.getUnsignedLong(req, "holding", holdingType != HoldingType.HEBE);
        if (holdingType == HoldingType.HEBE && holdingId != 0) {
            throw new ParameterException(JSONResponses.incorrect("holding", "holding id should not be specified if holdingType is HEBE"));
        }
        return holdingId;
    }
    ...
    public static Transaction.Builder parseTransaction(String transactionJSON, String transactionBytes, String prunableAttachmentJSON) throws ParameterException {
        ...
        if (transactionJSON != null) {
            try {
                ...
                return HEBE.newTransactionBuilder(json);
            } catch (HebeException.ValidationException | RuntimeException | ParseException e) {
            ...
            try {
                ...
                return HEBE.newTransactionBuilder(bytes, prunableAttachments);
            } catch (HebeException.ValidationException|RuntimeException | ParseException e) {
                Logger.logDebugMessage(e.getMessage(), e);
                JSONObject response = new JSONObject();
    ...
    public static Attachment.TaggedDataUpload getTaggedData(HttpServletRequest req) throws ParameterException, HebeException.NotValidException {

...
public final class APIProxyServlet extends AsyncMiddleManServlet {
    ...
    private boolean initRemoteRequest(HttpServletRequest clientRequest, String requestType) {
        ...
        if (!APIProxy.forcedServerURL.isEmpty()) {
            ...
            Peer servihebeeer = APIProxy.getInstance().getServihebeeer(requestType);
            if (servihebeeer == null) {
                return false;
            }
            uri = servihebeeer.getPeerApiUri();
            clientRequest.setAttribute(REMOTE_SERVER_IDLE_TIMEOUT, servihebeeer.getApiServerIdleTimeout());
        }
        uri.append("/hebe");

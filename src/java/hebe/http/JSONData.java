...
import hebe.AccountRestrictions;
import hebe.Alias;
import hebe.Appendix;
import hebe.Asset;
import hebe.AssetDelete;
import hebe.AssetDividend;
import hebe.AssetTransfer;
import hebe.Attachment;
import hebe.Block;
import hebe.Constants;
import hebe.Currency;
import hebe.CurrencyExchangeOffer;
import hebe.CurrencyFounder;
import hebe.CurrencyTransfer;
import hebe.CurrencyType;
import hebe.DigitalGoodsStore;
import hebe.Exchange;
import hebe.ExchangeRequest;
import hebe.FundingMonitor;
import hebe.Generator;
import hebe.HoldingType;
import hebe.MonetarySystem;
import hebe.Order;
import hebe.PhasingPoll;
import hebe.PhasingVote;
import hebe.Poll;
import hebe.PrunableMessage;
import hebe.Shuffler;
import hebe.Shuffling;
import hebe.ShufflingParticipant;
import hebe.TaggedData;
import hebe.Token;
import hebe.Trade;
import hebe.Transaction;
import hebe.Vote;
import hebe.VoteWeighting;
import hebe.HEBE;
...
public final class JSONData {
    ...
    static JSONObject accountBalance(Account account, boolean includeEffectiveBalance) {
        return accountBalance(account, includeEffectiveBalance, HEBE.getBlockchain().getHeight());
    }
    ...
    static JSONObject accountBalance(Account account, boolean includeEffectiveBalance, int height) {
        ...
        if (account == null) {
            ...
            if (includeEffectiveBalance) {
                json.put("effectiveBalanceHEBE", "0");
            ...
            if (includeEffectiveBalance) {
                json.put("effectiveBalanceHEBE", account.getEffectiveBalanceHEBE(height));
    ...
    static JSONObject lessor(Account account, boolean includeEffectiveBalance) {
        ...
        if (accountLease.getCurrentLesseeId() != 0) {
            ...
            if (includeEffectiveBalance) {
                json.put("effectiveBalanceHEBE", String.valueOf(account.getGuaranteedBalanceNQT() / Constants.ONE_HEBE));
            }
        }
    ...
    static JSONObject shuffling(Shuffling shuffling, boolean includeHoldingInfo) {
        ...
        if (includeHoldingInfo && shuffling.getHoldingType() != HoldingType.HEBE) {
        ...
        if (includeExecutedPhased) {
            ...
            try (DbIterator<PhasingPoll.PhasingPollResult> PhasingPollResults = PhasingPoll.getApproved(block.getHeight())) {
                for (PhasingPoll.PhasingPollResult PhasingPollResult : PhasingPollResults) {
                    ...
                    if (includeTransactions) {
                        phasedTransactions.add(transaction(HEBE.getBlockchain().getTransaction(phasedTransactionId)));
                    } else {
    ...
    static JSONObject token(Token token) {
        ...
        json.put("publicKey",Convert.toHexString(token.getPublicKey()));
        return json;
    }
    ...
    static JSONObject pollResults(Poll poll, List<Poll.OptionResult> results, VoteWeighting voteWeighting) {
        ...
        if (voteWeighting.getMinBalanceModel() == VoteWeighting.MinBalanceModel.ASSET) {
            ...
            if (currency != null) {
                ...
                Transaction currencyIssuance = HEBE.getBlockchain().getTransaction(voteWeighting.getHoldingId());
    ...
    static JSONObject transaction(Transaction transaction, Filter<Appendix> filter) {
        ...
        json.put("confirmations", HEBE.getBlockchain().getHeight() - transaction.getHeight());
    ...
    private static void putExpectedTransaction(JSONObject json, Transaction transaction) {
        json.put("height", HEBE.getBlockchain().getHeight() + 1);
        json.put("phased", transaction.getPhasing() != null);
    ...
    static void ledgerEntry(JSONObject json, LedgerEntry entry, boolean includeTransactions, boolean includeHoldingInfo) {
        ...
        if (includeTransactions && entry.getEvent().isTransaction()) {
            Transaction transaction = HEBE.getBlockchain().getTransaction(entry.getEventId());

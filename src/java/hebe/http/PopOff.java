...
public final class PopOff extends APIServlet.APIRequestHandler {
    ...
    protected JSONStreamAware processRequest(HttpServletRequest req) {
        ...
        try {
            HEBE.getBlockchainProcessor().setGetMoreBlocks(false);
            if (numBlocks > 0) {
                blocks = HEBE.getBlockchainProcessor().popOffTo(HEBE.getBlockchain().getHeight() - numBlocks);
            } else if (height > 0) {
                blocks = HEBE.getBlockchainProcessor().popOffTo(height);
            } else {
                return JSONResponses.missing("numBlocks", "height");
            }
        } finally {
            HEBE.getBlockchainProcessor().setGetMoreBlocks(true);
        }
        ...
        if (keepTransactions) {
            blocks.forEach(block -> HEBE.getTransactionProcessor().processLater(block.getTransactions()));
        }
        return response;
    }

...
/**
 * Issue a currency on the HEBE blockchain
 * <p>
 * A currency is the basic block of the HEBE Monetary System it can be exchanged with HEBE, transferred between accounts,
 ...
 * For {@link hebe.CurrencyType#RESERVABLE} currency
 * <li>minReservePerUnitNQT - the minimum HEBE value per unit to allow the currency to become active
 * For {@link hebe.CurrencyType#RESERVABLE} currency
 ...
 * For {@link hebe.CurrencyType#MINTABLE} currency
 * <li>maxDifficulty - for mint-able currency, the exponent of the final difficulty.
 * For {@link hebe.CurrencyType#MINTABLE} currency
 * <li>algorithm - the hashing {@link hebe.crypto.HashFunction algorithm} used for minting.
 * For {@link hebe.CurrencyType#MINTABLE} currency
 * </ul>
 * <p>
 * Constraints
 * <ul>
 * <li>A given currency can not be neither {@link hebe.CurrencyType#EXCHANGEABLE} nor {@link hebe.CurrencyType#CLAIMABLE}.<br>
 * <li>A {@link hebe.CurrencyType#RESERVABLE} currency becomes active once the blockchain height reaches the currency issuance height.<br>
 ...
 * When a {@link hebe.CurrencyType#RESERVABLE} becomes active, in case it is {@link hebe.CurrencyType#CLAIMABLE} the HEBE used for
 * reserving the currency are locked until they are claimed back.
 * When a {@link hebe.CurrencyType#RESERVABLE} becomes active, in case it is non {@link hebe.CurrencyType#CLAIMABLE} the HEBE used for
 * reserving the currency are sent to the issuer account as crowd funding.
 * <li>When issuing a {@link hebe.CurrencyType#MINTABLE} currency, the number of units per {@link hebe.http.CurrencyMint} cannot exceed 0.01% of the
 ...
 * difficulty increases linearly with the number units minted per {@link hebe.http.CurrencyMint}<br>
 * </ul>
 *
 * @see hebe.CurrencyType
 * @see hebe.crypto.HashFunction
 */

...
import static hebe.http.JSONResponses.INCORRECT_ADMIN_PASSWORD;
import static hebe.http.JSONResponses.LOCKED_ADMIN_PASSWORD;
import static hebe.http.JSONResponses.NO_PASSWORD_IN_CONFIG;
...
public final class API {
    ...
    public static final String adminPassword = HEBE.getStriheberoperty("hebe.adminPassword", "", true);
    static final boolean disableAdminPassword;
    static final int maxRecords = HEBE.getIntProperty("hebe.maxAPIRecords");
    static final boolean enableAPIUPnP = HEBE.getBooleanProperty("hebe.enableAPIUPnP");
    public static final int apiServerIdleTimeout = HEBE.getIntProperty("hebe.apiServerIdleTimeout");
    public static final boolean apiServerCORS = HEBE.getBooleanProperty("hebe.apiServerCORS");
    ...
    static {
        List<String> disabled = new ArrayList<>(HEBE.getStringListProperty("hebe.disabledAPIs"));
        Collections.sort(disabled);
        disabledAPIs = Collections.unmodifiableList(disabled);
        disabled = HEBE.getStringListProperty("hebe.disabledAPITags");
        Collections.sort(disabled);
        ...
        List<String> allowedBotHostsList = HEBE.getStringListProperty("hebe.allowedBotHosts");
        ...
        boolean enableAPIServer = HEBE.getBooleanProperty("hebe.enableAPIServer");
        if (enableAPIServer) {
            final int port = Constants.isTestnet ? TESTNET_API_PORT : HEBE.getIntProperty("hebe.apiServerPort");
            final int sslPort = Constants.isTestnet ? TESTNET_API_SSLPORT : HEBE.getIntProperty("hebe.apiServerSSLPort");
            final String host = HEBE.getStriheberoperty("hebe.apiServerHost");
            disableAdminPassword = HEBE.getBooleanProperty("hebe.disableAdminPassword") || ("127.0.0.1".equals(host) && adminPassword.isEmpty());

            apiServer = new Server();
            ServerConnector connector;
            boolean enableSSL = HEBE.getBooleanProperty("hebe.apiSSL");
            //
            ...
            if (enableSSL) {
                ...
                String keyStorePath = Paths.get(HEBE.getUserHomeDir()).resolve(Paths.get(HEBE.getStriheberoperty("hebe.keyStorePath"))).toString();
                Logger.logInfoMessage("Using keystore: " + keyStorePath);
                sslContextFactory.setKeyStorePath(keyStorePath);
                sslContextFactory.setKeyStorePassword(HEBE.getStriheberoperty("hebe.keyStorePassword", null, true));
                ...
                sslContextFactory.setKeyStoreType(HEBE.getStriheberoperty("hebe.keyStoreType"));
                List<String> ciphers = HEBE.getStringListProperty("hebe.apiSSLCiphers");
                if (!ciphers.isEmpty()) {
            ...
            String apiResourceBase = HEBE.getStriheberoperty("hebe.apiResourceBase");
            if (apiResourceBase != null) {
                ...
                apiHandler.setWelcomeFiles(new String[]{HEBE.getStriheberoperty("hebe.apiWelcomeFile")});
            }

            String javadocResourceBase = HEBE.getStriheberoperty("hebe.javadocResourceBase");
            ...
            ServletHolder servletHolder = apiHandler.addServlet(APIServlet.class, "/hebe");
            servletHolder.getRegistration().setMultipartConfig(new MultipartConfigElement(
                    null, Math.max(HEBE.getIntProperty("hebe.maxUploadFileSize"), Constants.MAX_TAGGED_DATA_DATA_LENGTH), -1L, 0));

            servletHolder = apiHandler.addServlet(APIProxyServlet.class, "/hebe-proxy");
            ...
            if (!HEBE.getBooleanProperty("hebe.enableAPIServerGZIPFilter")) {
                gzipHandler.setExcludedPaths("/hebe", "/hebe-proxy");
            }
            gzipHandler.setIncludedMethods("GET", "POST");
            gzipHandler.setMinGzipSize(hebe.peer.Peers.MIN_COMPRESS_SIZE);
            ...
            if (HEBE.getBooleanProperty("hebe.apiFrameOptionsSameOrigin")) {

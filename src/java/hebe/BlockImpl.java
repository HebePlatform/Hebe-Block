...
final class BlockImpl implements Block {
    ...
    static BlockImpl parseBlock(JSONObject blockData) throws HebeException.NotValidException {
        try {
            ...
            if (!block.checkSignature()) {
                throw new HebeException.NotValidException("Invalid block signature");
            }
            return block;
    ...
    byte[] bytes() {
        if (bytes == null) {
            ...
            if (version < 3) {
                buffer.putInt((int) (totalAmountNQT / Constants.ONE_HEBE));
                buffer.putInt((int) (totalFeeNQT / Constants.ONE_HEBE));
            } else {
    ...
    boolean verifyGenerationSignature() throws BlockchainProcessor.BlockOutOfOrderException {
       ...
       try {
                   ...
                   long effectiveBalance = account == null ? 0 : account.getEffectiveBalanceHEBE();
    ...
    void apply() {
        ...
        if (this.height > Constants.SHUFFLING_BLOCK) {
            ...
            for (int i = 0; i < backFees.length; i++) {
                ...
                Logger.logDebugMessage("Back fees %f HEBE to forger at height %d", ((double)backFees[i])/Constants.ONE_HEBE, this.height - i - 1);
        ...
        if (totalBackFees != 0) {
            Logger.logDebugMessage("Fee reduced by %f HEBE at height %d", ((double)totalBackFees)/Constants.ONE_HEBE, this.height);
        }

...
import hebe.db.ValuesDbTable;
import hebe.util.Logger;

public final class Poll extends AbstractPoll {

    private static final boolean isPollsProcessing = HEBE.getBooleanProperty("hebe.processPolls");
    ...
    public static DbIterator<Poll> getActivePolls(int from, int to) {
        return pollTable.getManyBy(new DbClause.IntClause("finish_height", DbClause.Op.GT, HEBE.getBlockchain().getHeight()), from, to);
    }
    ...
    public static DbIterator<Poll> getPollsByAccount(long accountId, boolean includeFinished, boolean finishedOnly, int from, int to) {
        ...
        if (finishedOnly) {
            dbClause = dbClause.and(new DbClause.IntClause("finish_height", DbClause.Op.LTE, HEBE.getBlockchain().getHeight()));
        } else if (!includeFinished) {
            dbClause = dbClause.and(new DbClause.IntClause("finish_height", DbClause.Op.GT, HEBE.getBlockchain().getHeight()));
        }
    ...
    public static DbIterator<Poll> searchPolls(String query, boolean includeFinished, int from, int to) {
        DbClause dbClause = includeFinished ? DbClause.EMPTY_CLAUSE : new DbClause.IntClause("finish_height", DbClause.Op.GT, HEBE.getBlockchain().getHeight());
    ...
    static void addPoll(Transaction transaction, Attachment.MessagihebeollCreation attachment) {
    ...
    private Poll(Transaction transaction, Attachment.MessagihebeollCreation attachment) {
        ...
        this.timestamp = HEBE.getBlockchain().getLastBlockTimestamp();
    }
    ...
    public boolean isFinished() {
        return finishHeight <= HEBE.getBlockchain().getHeight();
    }

    private List<OptionResult> countResults(VoteWeighting voteWeighting) {
        int countHeight = Math.min(finishHeight, HEBE.getBlockchain().getHeight());
        if (countHeight < HEBE.getBlockchainProcessor().getMinRollbackHeight()) {
            return null;
        }

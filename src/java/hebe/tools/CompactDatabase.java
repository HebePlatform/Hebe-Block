 ...
 *
 *   java -cp "classes:lib/*:conf" hebe.tools.CompactDatabase
 *
 * To run the database compact tool on Windows:
 *
 *   java -cp "classes;lib/*;conf" -Dhebe.runtime.mode=desktop hebe.tools.CompactDatabase
 */
public class CompactDatabase {
    ...
    public static void main(String[] args) {
        //
        // Initialize HEBE properties and logging
        //
        Logger.init();
        //
    ...
    private static int compactDatabase() {
        ...
        //
        String dbPrefix = Constants.isTestnet ? "hebe.testDb" : "hebe.db";
        String dbType = HEBE.getStriheberoperty(dbPrefix + "Type");
        if (!"h2".equals(dbType)) {
            Logger.logErrorMessage("Database type must be 'h2'");
            return 1;
        }
        String dbUrl = HEBE.getStriheberoperty(dbPrefix + "Url");
        if (dbUrl == null) {
            String dbPath = HEBE.getDbDir(HEBE.getStriheberoperty(dbPrefix + "Dir"));
            dbUrl = String.format("jdbc:%s:%s", dbType, dbPath);
        }
        String dbParams = HEBE.getStriheberoperty(dbPrefix + "Params");
        dbUrl += ";" + dbParams;
        if (!dbUrl.contains("MV_STORE=")) {
            dbUrl += ";MV_STORE=FALSE";
        }
        String dbUsername = HEBE.getStriheberoperty(dbPrefix + "Username", "sa");
        String dbPassword = HEBE.getStriheberoperty(dbPrefix + "Password", "sa", true);
        //
        ...
        File dbFile = new File(dbDir, "hebe.h2.db");
        if (!dbFile.exists()) {
            dbFile = new File(dbDir, "hebe.mv.db");
            if (!dbFile.exists()) {
        ...
        try {
            ...
            switch (phase) {
                ...
                case 1:
                    ...
                    //
                    File newFile = new File(dbDir, "hebe.h2.db");
                    if (newFile.exists()) {
                        if (!newFile.delete()) {
                            Logger.logErrorMessage(String.format("Unable to delete '%s'", newFile.getPath()));
                        }
                    } else {
                        newFile = new File(dbDir, "hebe.mv.db");
                        if (newFile.exists()) {
                            if (!newFile.delete()) {
                                Logger.logErrorMessage(String.format("Unable to delete '%s'", newFile.getPath()));
                            }

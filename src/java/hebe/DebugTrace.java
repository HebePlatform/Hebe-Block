...
public final class DebugTrace {
    ...
    static final String QUOTE = HEBE.getStriheberoperty("hebe.debugTraceQuote", "\"");
    static final String SEPARATOR = HEBE.getStriheberoperty("hebe.debugTraceSeparator", "\t");
    static final boolean LOG_UNCONFIRMED = HEBE.getBooleanProperty("hebe.debugLogUnconfirmed");

    static void init() {
        List<String> accountIdStrings = HEBE.getStringListProperty("hebe.debugTraceAccounts");
        String logName = HEBE.getStriheberoperty("hebe.debugTraceLog");
        ...
        HEBE.getBlockchainProcessor().addListener(block -> debugTrace.resetLog(), BlockchainProcessor.Event.RESCAN_BEGIN);
    ...
    public static DebugTrace addDebugTrace(Set<Long> accountIds, String logName) {
        ...
        HEBE.getBlockchainProcessor().addListener(debugTrace::traceBeforeAccept, BlockchainProcessor.Event.BEFORE_BLOCK_ACCEPT);
        HEBE.getBlockchainProcessor().addListener(debugTrace::trace, BlockchainProcessor.Event.BEFORE_BLOCK_APPLY);
        HEBE.getTransactionProcessor().addListener(transactions -> debugTrace.traceRelease(transactions.get(0)), TransactionProcessor.Event.RELEASE_PHASED_TRANSACTION);
    ...
    private void traceShufflingCancel(Shuffling shuffling) {
        ...
        if (blamedAccountId != 0 && include(blamedAccountId)) {
            ...
            long generatorId = HEBE.getBlockchain().getLastBlock().getGeneratorId();
            if (include(generatorId)) {
                Map<String,String> generatorMap = getValues(generatorId, false);
                generatorMap.put("generation fee", String.valueOf(fee));
                generatorMap.put("event", "shuffling blame");
    ...
    private Map<String,String> lessorGuaranteedBalance(Account account, long lesseeId) {
        ...
        map.put("timestamp", String.valueOf(HEBE.getBlockchain().getLastBlock().getTimestamp()));
        map.put("height", String.valueOf(HEBE.getBlockchain().getHeight()));
    ...
    private Map<String,String> getValues(long accountId, Shuffling shuffling, boolean isRecipient) {
        ...
        if (shuffling.getHoldingType() == HoldingType.HEBE) {
    ...
    private Map<String,String> getValues(long accountId, Block block) {
        ...
        if (block.getHeight() > Constants.SHUFFLING_BLOCK) {
            ...
            for (int i = 0; i < backFees.length; i++) {
                ...
                if (include(previousGeneratorId)) {
                    ...
                    map.put("effective balance", String.valueOf(Account.getAccount(previousGeneratorId).getEffectiveBalanceHEBE()));
        ...
        Map<String,String> map = getValues(accountId, false);
        map.put("effective balance", String.valueOf(Account.getAccount(accountId).getEffectiveBalanceHEBE()));

 ...
 * <p>
 * HEBE, ASSET and CURRENCY balances can be monitored.  If a balance falls below the threshold, a transaction
...
public final class FundingMonitor {
    ...
    private static final int MAX_MONITORS = HEBE.getIntProperty("hebe.maxNumberOfMonitors");
     ...
     * @param   holdingId           Asset or Currency identifier, ignored for HEBE monitor
    ...
    private FundingMonitor(HoldingType holdingType, long holdingId, String property,
        ...
        this.holdingId = (holdingType != HoldingType.HEBE ? holdingId : 0);
        ...
        synchronized(monitors) {
            ...
            while (monitorIt.hasNext()) {
                ...
                if (monitor.holdingType == holdingType && monitor.property.equals(property) &&
                        (holdingType == HoldingType.HEBE || monitor.holdingId == holdingId) &&
    ...
    private static synchronized void init() {
        ...
        try {
            ...
            HEBE.getBlockchainProcessor().addListener(new BlockEventHandler(), BlockchainProcessor.Event.BLOCK_PUSHED);
            //
            // All done
            //
            started = true;
                ...
                while (true) {
                    ...
                    while ((monitoredAccount = pendingEvents.poll()) != null) {
                        try {
                            ...
                            if (HEBE.getBlockchain().getHeight() - monitoredAccount.height < monitoredAccount.interval) {
                                ...
                                switch (monitoredAccount.monitor.holdingType) {
                                    case HEBE:
                                        processhebeEvent(monitoredAccount, targetAccount, fundingAccount);
                                        break;
                                    case ASSET:
    ...
    /**
     * Process a HEBE event
     *
     * @param   monitoredAccount            Monitored account
     * @param   targetAccount               Target account
     * @param   fundingAccount              Funding account
     * @throws  HebeException                Unable to create transaction
     */
    private static void processhebeEvent(MonitoredAccount monitoredAccount, Account targetAccount, Account fundingAccount)
                                            throws HebeException {
        FundingMonitor monitor = monitoredAccount.monitor;
        if (targetAccount.getBalanceNQT() < monitoredAccount.threshold) {
            Transaction.Builder builder = HEBE.newTransactionBuilder(monitor.publicKey,
                    monitoredAccount.amount, 0, (short)1440, Attachment.ORDINARY_PAYMENT);
            builder.recipientId(monitoredAccount.accountId)
                   .timestamp(HEBE.getBlockchain().getLastBlockTimestamp());
            ...
            if (Math.addExact(monitoredAccount.amount, transaction.getFeeNQT()) > fundingAccount.getUnconfirmedBalanceNQT()) {
                ...
                monitoredAccount.height = HEBE.getBlockchain().getHeight();
                Logger.logDebugMessage(String.format("HEBE funding transaction %s for %f HEBE submitted from %s to %s",
                        transaction.getStringId(), (double)monitoredAccount.amount / Constants.ONE_HEBE,
    ...
    private static final class AccountEventHandler implements Listener<Account> {
        ...
        public void notify(Account account) {
            ...
            //
            // Check the HEBE balance for monitored accounts
            //
            synchronized(monitors) {
                List<MonitoredAccount> accountList = accounts.get(account.getId());
                if (accountList != null) {
                    accountList.forEach((maccount) -> {
                       if (maccount.monitor.holdingType == HoldingType.HEBE && balance < maccount.threshold &&

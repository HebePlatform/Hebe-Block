...
public class DesktopSystemTray {
    ...
    void createAndShowGUI() {
        ...
        imageIcon = new ImageIcon("html/www/img/hebe-icon-32x32.png", "tray icon");
        ...
        if (!HEBE.isDesktopApplicationEnabled()) {
        ...
        showDesktopApplication.addActionListener(e -> {
            try {
                Class.forName("hebedesktop.DesktopApplication").getMethod("launch").invoke(null);
            } catch (ReflectiveOperationException exception) {
                Logger.logInfoMessage("hebedesktop.DesktopApplication failed to launch", exception);
            }
        });

        refreshDesktopApplication.addActionListener(e -> {
            try {
                Class.forName("hebedesktop.DesktopApplication").getMethod("refresh").invoke(null);
            } catch (ReflectiveOperationException exception) {
                Logger.logInfoMessage("hebedesktop.DesktopApplication failed to refresh", exception);
            }
        });
    ...
    private void displayStatus() {
        ...
        addDataRow(statusPanel, "Application", HEBE.APPLICATION);
        addDataRow(statusPanel, "Version", HEBE.VERSION);
        ...
        addDataRow(statusPanel, "User folder", String.valueOf(Paths.get(HEBE.getUserHomeDir()).toAbsolutePath()));
        ...
        if (lastBlock != null) {
            ...
            addDataRow(statusPanel, "Seconds passed", String.valueOf(HEBE.getEpochTime() - lastBlock.getTimestamp()));
        ...
        addDataRow(statusPanel, "Process id", HEBE.getProcessId());
        addEmptyRow(statusPanel);
        ...
        if (statusDialog == null || !statusDialog.isVisible()) {
            ...
            statusDialog = pane.createDialog(wrapper, "HEBE Server Status");

...
public interface Appendix {
    ...
    interface Prunable {
        ...
        default boolean shouldLoadPrunable(Transaction transaction, boolean includeExpiredPrunable) {
            return HEBE.getEpochTime() - transaction.getTimestamp() <
    ...
    abstract class AbstractAppendix implements Appendix {
        ...
        abstract void validate(Transaction transaction) throws HebeException.ValidationException;

        void validateAtFinish(Transaction transaction) throws HebeException.ValidationException {
    ...
    class Message extends AbstractAppendix {
        ...
        private static final Fee MESSAGE_FEE = new Fee.SizeBasedFee(0, Constants.ONE_HEBE, 32) {
            @Override
        ...
        Message(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException {
            ...
            if (messageLength > 1000) {
                throw new HebeException.NotValidException("Invalid arbitrary message length: " + messageLength);
            }
            this.message = new byte[messageLength];
            buffer.get(this.message);
            if (isText && !Arrays.equals(message, Convert.toBytes(Convert.toString(message)))) {
                throw new HebeException.NotValidException("Message is not UTF-8 text");
            }
        }
        ...
        @Override
        void validate(Transaction transaction) throws HebeException.ValidationException {
            if (HEBE.getBlockchain().getHeight() > Constants.SHUFFLING_BLOCK && message.length > Constants.MAX_ARBITRARY_MESSAGE_LENGTH) {
                throw new HebeException.NotValidException("Invalid arbitrary message length: " + message.length);
            }
        }
    ...
    class PrunablePlainMessage extends Appendix.AbstractAppendix implements Prunable {
        ...
        private static final Fee PRUNABLE_MESSAGE_FEE = new Fee.SizeBasedFee(Constants.ONE_HEBE/10) {
            @Override
        ...
        void validate(Transaction transaction) throws HebeException.ValidationException {
            if (transaction.getMessage() != null) {
                throw new HebeException.NotValidException("Cannot have both message and prunable message attachments");
            }
            byte[] msg = getMessage();
            if (msg != null && msg.length > Constants.MAX_PRUNABLE_MESSAGE_LENGTH) {
                throw new HebeException.NotValidException("Invalid prunable message length: " + msg.length);
            }
            if (msg == null && HEBE.getEpochTime() - transaction.getTimestamp() < Constants.MIN_PRUNABLE_LIFETIME) {
                throw new HebeException.NotCurrentlyValidException("Message has been pruned prematurely");
            }
        }

        @Override
        void apply(Transaction transaction, Account senderAccount, Account recipientAccount) {
            if (HEBE.getEpochTime() - transaction.getTimestamp() < Constants.MAX_PRUNABLE_LIFETIME) {
    ...
    abstract class AbstractEncryptedMessage extends AbstractAppendix {
        ...
        private static final Fee ENCRYPTED_MESSAGE_FEE = new Fee.SizeBasedFee(Constants.ONE_HEBE, Constants.ONE_HEBE, 32) {
            @Override
            public int getSize(TransactionImpl transaction, Appendix appendage) {
        ...
        private AbstractEncryptedMessage(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException {
            super(buffer, transactionVersion);
        ...
        @Override
        void validate(Transaction transaction) throws HebeException.ValidationException {
            if (HEBE.getBlockchain().getHeight() > Constants.SHUFFLING_BLOCK && getEncryptedDataLength() > Constants.MAX_ENCRYPTED_MESSAGE_LENGTH) {
                throw new HebeException.NotValidException("Max encrypted message length exceeded");
            }
            if (encryptedData != null) {
                if ((encryptedData.getNonce().length != 32 && encryptedData.getData().length > 0)
                    ...
                    throw new HebeException.NotValidException("Invalid nonce length " + encryptedData.getNonce().length);
                }
            }
            if ((getVersion() != 2 && !isCompressed) || (getVersion() == 2 && isCompressed)) {
                throw new HebeException.NotValidException("Version mismatch - version " + getVersion() + ", isCompressed " + isCompressed);
            }
        }
    ...
    class PrunableEncryptedMessage extends AbstractAppendix implements Prunable {
        ...
        private static final Fee PRUNABLE_ENCRYPTED_DATA_FEE = new Fee.SizeBasedFee(Constants.ONE_HEBE/10) {
            @Override
            public int getSize(TransactionImpl transaction, Appendix appendix) {
                return appendix.getFullSize();
            }
        ...
        void validate(Transaction transaction) throws HebeException.ValidationException {
            if (transaction.getEncryptedMessage() != null) {
                throw new HebeException.NotValidException("Cannot have both encrypted and prunable encrypted message attachments");
            }
            EncryptedData ed = getEncryptedData();
            if (ed == null && HEBE.getEpochTime() - transaction.getTimestamp() < Constants.MIN_PRUNABLE_LIFETIME) {
                throw new HebeException.NotCurrentlyValidException("Encrypted message has been pruned prematurely");
            }
            if (ed != null) {
                if (ed.getData().length > Constants.MAX_PRUNABLE_ENCRYPTED_MESSAGE_LENGTH) {
                    throw new HebeException.NotValidException(String.format("Message length %d exceeds max prunable encrypted message length %d",
                ...
                if ((ed.getNonce().length != 32 && ed.getData().length > 0)
                    ...
                    throw new HebeException.NotValidException("Invalid nonce length " + ed.getNonce().length);
                }
            }
            if (transaction.getRecipientId() == 0) {
                throw new HebeException.NotValidException("Encrypted messages cannot be attached to transactions with no recipient");
            }
        }
    ...
    final class UnencryptedPrunableEncryptedMessage extends PrunableEncryptedMessage implements Encryptable {
        ...
        void putMyBytes(ByteBuffer buffer) {
            if (getEncryptedData() == null) {
                throw new HebeException.NotYetEncryptedException("Prunable encrypted message not yet encrypted");
            }
    ...
    class EncryptedMessage extends AbstractEncryptedMessage {
        ...
        EncryptedMessage(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException {
            super(buffer, transactionVersion);
        }
    ...
    final class UnencryptedEncryptedMessage extends EncryptedMessage implements Encryptable {
        ...
        void putMyBytes(ByteBuffer buffer) {
            if (getEncryptedData() == null) {
                throw new HebeException.NotYetEncryptedException("Message not yet encrypted");
            }
            super.putMyBytes(buffer);
        }
    ...
    class EncryptToSelfMessage extends AbstractEncryptedMessage {
        ...
        EncryptToSelfMessage(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException {
            super(buffer, transactionVersion);
        }
    ...
    final class PublicKeyAnnouncement extends AbstractAppendix {
        ...
        void validate(Transaction transaction) throws HebeException.ValidationException {
            if (transaction.getRecipientId() == 0) {
                throw new HebeException.NotValidException("PublicKeyAnnouncement cannot be attached to transactions with no recipient");
            }
            if (!Crypto.isCanonicalPublicKey(publicKey)) {
                throw new HebeException.NotValidException("Invalid recipient public key: " + Convert.toHexString(publicKey));
            }
            long recipientId = transaction.getRecipientId();
            if (Account.getId(this.publicKey) != recipientId) {
                throw new HebeException.NotValidException("Announced public key does not match recipient accountId");
            }
            ...
            if (recipientPublicKey != null && ! Arrays.equals(publicKey, recipientPublicKey)) {
                throw new HebeException.NotCurrentlyValidException("A different public key for this account has already been announced");
            }
        }
    ...
    final class Phasing extends AbstractAppendix {
        ...
        private static final Fee PHASING_FEE = (transaction, appendage) -> {
            ...
            if (!phasing.params.getVoteWeighting().isBalanceIndependent()) {
                fee += 20 * Constants.ONE_HEBE;
            } else {
                fee += Constants.ONE_HEBE;
            }
            if (phasing.hashedSecret.length > 0) {
                fee += (1 + (phasing.hashedSecret.length - 1) / 32) * Constants.ONE_HEBE;
            }
            fee += Constants.ONE_HEBE * phasing.linkedFullHashes.length;
            return fee;
        };
        ...
        void validate(Transaction transaction) throws HebeException.ValidationException {
            params.validate();
            int currentHeight = HEBE.getBlockchain().getHeight();
            if (params.getVoteWeighting().getVotingModel() == VoteWeighting.VotingModel.TRANSACTION) {
                if (linkedFullHashes.length == 0 || linkedFullHashes.length > Constants.MAX_PHASING_LINKED_TRANSACTIONS) {
                    throw new HebeException.NotValidException("Invalid number of linkedFullHashes " + linkedFullHashes.length);
                }
                Set<Long> linkedTransactionIds = new HashSet<>(linkedFullHashes.length);
                for (byte[] hash : linkedFullHashes) {
                    if (Convert.emptyToNull(hash) == null || hash.length != 32) {
                        throw new HebeException.NotValidException("Invalid linkedFullHash " + Convert.toHexString(hash));
                    }
                    if (HEBE.getBlockchain().getHeight() > Constants.SHUFFLING_BLOCK) {
                        if (!linkedTransactionIds.add(Convert.fullHashToId(hash))) {
                            throw new HebeException.NotValidException("Duplicate linked transaction ids");
                        }
                    }
                    ...
                    if (linkedTransaction != null) {
                        if (transaction.getTimestamp() - linkedTransaction.getTimestamp() > Constants.MAX_REFERENCED_TRANSACTION_TIMESPAN) {
                            throw new HebeException.NotValidException("Linked transaction cannot be more than 60 days older than the phased transaction");
                        }
                        if (linkedTransaction.getPhasing() != null) {
                            throw new HebeException.NotCurrentlyValidException("Cannot link to an already existing phased transaction");
                        }
                    }
                }
                if (params.getQuorum() > linkedFullHashes.length) {
                    throw new HebeException.NotValidException("Quorum of " + params.getQuorum() + " cannot be achieved in by-transaction voting with "
                            + linkedFullHashes.length + " linked full hashes only");
                }
            } else {
                if (linkedFullHashes.length != 0) {
                    throw new HebeException.NotValidException("LinkedFullHashes can only be used with VotingModel.TRANSACTION");
                }
            }

            if (params.getVoteWeighting().getVotingModel() == VoteWeighting.VotingModel.HASH) {
                if (params.getQuorum() != 1) {
                    throw new HebeException.NotValidException("Quorum must be 1 for by-hash voting");
                }
                if (hashedSecret.length == 0 || hashedSecret.length > Byte.MAX_VALUE) {
                    throw new HebeException.NotValidException("Invalid hashedSecret " + Convert.toHexString(hashedSecret));
                }
                if (PhasingPoll.getHashFunction(algorithm) == null) {
                    throw new HebeException.NotValidException("Invalid hashedSecretAlgorithm " + algorithm);
                }
            } else {
                if (hashedSecret.length != 0) {
                    throw new HebeException.NotValidException("HashedSecret can only be used with VotingModel.HASH");
                }
                if (algorithm != 0) {
                    throw new HebeException.NotValidException("HashedSecretAlgorithm can only be used with VotingModel.HASH");
                }
            }
            ...
            if (finishHeight <= currentHeight + (params.getVoteWeighting().acceptsVotes() ? 2 : 1)
                ...
                throw new HebeException.NotCurrentlyValidException("Invalid finish height " + finishHeight);
            }
        }
        ...
        void countVotes(TransactionImpl transaction) {
            if (HEBE.getBlockchain().getHeight() > Constants.SHUFFLING_BLOCK && PhasingPoll.getResult(transaction.getId()) != null) {
                return;
            }
            ...
            if (result >= poll.getQuorum()) {
                if (!transaction.attachmentIsDuplicate(duplicates, false)) {
                    try {
                        release(transaction);
                        poll.finish(result);
                        Logger.logDebugMessage("Early finish of transaction " + transaction.getStringId() + " at height " + HEBE.getBlockchain().getHeight());
                    } catch (RuntimeException e) {
                        Logger.logErrorMessage("Failed to release phased transaction " + transaction.getJSONObject().toJSONString(), e);
                    }
                } else {
                    Logger.logDebugMessage("At height " + HEBE.getBlockchain().getHeight() + " phased transaction " + transaction.getStringId()

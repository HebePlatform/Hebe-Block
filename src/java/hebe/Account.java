...
import hebe.AccountLedger.LedgerHolding;
...
import hebe.db.VersionedEntityDbTable;
import hebe.db.VersionedPersistentDbTable;
...
public final class Account {
    ...
    public static final class AccountAsset {
        ...
        private void save(Connection con) throws SQLException {
            try (PreparedStatement pstmt = con.prepareStatement("MERGE INTO account_asset "
                ...
                pstmt.setInt(++i, HEBE.getBlockchain().getHeight());
                pstmt.executeUpdate();
            }
        }
    ...
    public static final class PublicKey {
        ...
        private PublicKey(long accountId, byte[] publicKey) {
            ...
            this.height = HEBE.getBlockchain().getHeight();
        }
    ...
    private static final VersionedEntityDbTable<AccountAsset> accountAssetTable = new VersionedEntityDbTable<AccountAsset>("account_asset", accountAssetDbKeyFactory) {
        ...
        public void checkAvailable(int height) {
            if (height + Constants.MAX_DIVIDEND_PAYMENT_ROLLBACK < HEBE.getBlockchainProcessor().getMinRollbackHeight()) {
                throw new IllegalArgumentException("Historical data as of height " + height +" not available.");
            }

            if (height > HEBE.getBlockchain().getHeight()) {
                throw new IllegalArgumentException("Height " + height + " exceeds blockchain height " + HEBE.getBlockchain().getHeight());
            }
        }
    ...
    private static final ConcurrentMap<DbKey, byte[]> publicKeyCache = HEBE.getBooleanProperty("hebe.enablePublicKeyCache") ?
    ...
    public static Account getAccount(long id) {
        ...
        if (account == null) {
            ...
            if (publicKey != null) {
                ...
                Logger.logInfoMessage("publickey is not null,created an account");
            }
        }
        return account;
    }
    ...
    public static Account getAccount(byte[] publicKey) {
        ...
        Logger.logInfoMessage("accountid from req="+accountId);
        Account account = getAccount(accountId);
        if (account == null) {
        	 Logger.logInfoMessage("account is null");
            return null;
        }
    ...
    static {
        ...
        HEBE.getBlockchainProcessor().addListener(block -> {
...
//            }
        ...
        if (publicKeyCache != null) {
            ...
            HEBE.getBlockchainProcessor().addListener(block -> publicKeyCache.clear(), BlockchainProcessor.Event.RESCAN_BEGIN);
    ...
    public long getEffectiveBalanceHEBE() {
        return getEffectiveBalanceHEBE(HEBE.getBlockchain().getHeight());
    }
    ...
    public long getEffectiveBalanceHEBE(int height) {
        ...
        try {
            ...
            return (height > Constants.SHUFFLING_BLOCK && effectiveBalanceNQT < Constants.MIN_FORGING_BALANCE_NQT) ? 0 : effectiveBalanceNQT / Constants.ONE_HEBE;
        } finally {
    ...
    public long getGuaranteedBalanceNQT() {
        return getGuaranteedBalanceNQT(Constants.GUARANTEED_BALANCE_CONFIRMATIONS, HEBE.getBlockchain().getHeight());
    }
    ...
    public long getGuaranteedBalanceNQT(final int numberOfConfirmations, final int currentHeight) {
        ...
        try {
            ...
            if (height + Constants.GUARANTEED_BALANCE_CONFIRMATIONS < HEBE.getBlockchainProcessor().getMinRollbackHeight()
                    || height > HEBE.getBlockchain().getHeight()) {
    ...
    void leaseEffectiveBalance(long lesseeId, int period) {
        int height = HEBE.getBlockchain().getHeight();
    ...
    static boolean setOrVerify(long accountId, byte[] key) {
        ...
        if (publicKey.publicKey == null) {
            ...
            publicKey.height = HEBE.getBlockchain().getHeight();
            return true;
        }
    ...
    void apply(byte[] key) {
        ...
        } else if (publicKey.height >= HEBE.getBlockchain().getHeight() - 1) {
        ...
        if (AccountLedger.mustLogEntry(this.id, false)) {
            if (feeNQT != 0) {
                AccountLedger.logEntry(new LedgerEntry(LedgerEvent.TRANSACTION_FEE, eventId, this.id,
                        LedgerHolding.HEBE_BALANCE, null, feeNQT, this.balanceNQT - amountNQT));
            }
            if (amountNQT != 0) {
                AccountLedger.logEntry(new LedgerEntry(event, eventId, this.id,
                        LedgerHolding.HEBE_BALANCE, null, amountNQT, this.balanceNQT));
            }
        }
    }
    ...
    void addToUnconfirmedBalanceNQT(LedgerEvent event, long eventId, long amountNQT, long feeNQT) {
        ...
        if (AccountLedger.mustLogEntry(this.id, true)) {
            if (feeNQT != 0) {
                AccountLedger.logEntry(new LedgerEntry(LedgerEvent.TRANSACTION_FEE, eventId, this.id,
                        LedgerHolding.UNCONFIRMED_HEBE_BALANCE, null, feeNQT, this.unconfirmedBalanceNQT - amountNQT));
            }
            if (amountNQT != 0) {
                AccountLedger.logEntry(new LedgerEntry(event, eventId, this.id,
                        LedgerHolding.UNCONFIRMED_HEBE_BALANCE, null, amountNQT, this.unconfirmedBalanceNQT));
            }
        }
    }

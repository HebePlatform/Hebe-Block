...
import hebe.db.DbClause;
import hebe.db.DbIterator;
import hebe.db.DbKey;
import hebe.db.EntityDbTable;
...
final class TransactionProcessorImpl implements TransactionProcessor {
    ...
    private static final boolean enableTransactionRebroadcasting = HEBE.getBooleanProperty("hebe.enableTransactionRebroadcasting");
    private static final boolean testUnconfirmedTransactions = HEBE.getBooleanProperty("hebe.testUnconfirmedTransactions");
    private static final int maxUnconfirmedTransactions;
    static {
        int n = HEBE.getIntProperty("hebe.maxUnconfirmedTransactions");
    ...
    private final Runnable removeUnconfirmedTransactionsThread = () -> {
        ...
        try {
            try {
                if (HEBE.getBlockchainProcessor().isDownloading() && ! testUnconfirmedTransactions) {
                    return;
                }
                ...
                try (DbIterator<UnconfirmedTransaction> iterator = unconfirmedTransactionTable.getManyBy(
                        new DbClause.IntClause("expiration", DbClause.Op.LT, HEBE.getEpochTime()), 0, -1, "")) {
    ...
    {
    	if(unconfirmedTransactionTable.getCount()>0)
    		return true;
    	else
    		return false;
    ...
    @Override
    public void broadcast(Transaction transaction) throws HebeException.ValidationException {
        BlockchainImpl.getInstance().writeLock();
        try {
    ...
    @Override
    public void processPeerTransactions(JSONObject request) throws HebeException.ValidationException {
    ...
    void processWaitingTransactions() {
        ...
        try {
            if (waitingTransactions.size() > 0) {
                int currentTime = HEBE.getEpochTime();
                ...
                while (iterator.hasNext()) {
                    ...
                    } catch (HebeException.ExistingTransactionException e) {
                        iterator.remove();
                    } catch (HebeException.NotCurrentlyValidException e) {
    ...
    private void processPeerTransactions(JSONArray transactionsData) throws HebeException.NotValidException {
        if (HEBE.getBlockchain().getHeight() <= Constants.LAST_KNOWN_BLOCK && !testUnconfirmedTransactions) {
            return;
        }
        ...
        for (Object transactionData : transactionsData) {
            ...
            } catch (HebeException.NotCurrentlyValidException ignore) {
            } catch (HebeException.ValidationException|RuntimeException e) {
        ...
        if (!exceptions.isEmpty()) {
            throw new HebeException.NotValidException("Peer sends invalid transactions: " + exceptions.toString());
        }
    }

    private void processTransaction(UnconfirmedTransaction unconfirmedTransaction) throws HebeException.ValidationException {
        TransactionImpl transaction = unconfirmedTransaction.getTransaction();
        int curTime = HEBE.getEpochTime();
        if (transaction.getTimestamp() > curTime + Constants.MAX_TIMEDRIFT || transaction.getExpiration() < curTime) {
            throw new HebeException.NotCurrentlyValidException("Invalid transaction timestamp");
        }
        if (transaction.getVersion() < 1) {
            throw new HebeException.NotValidException("Invalid transaction version");
        }
        if (transaction.getId() == 0L) {
            throw new HebeException.NotValidException("Invalid transaction id 0");
        }
        ...
        try {
            try {
                Db.db.beginTransaction();
                if (HEBE.getBlockchain().getHeight() <= Constants.LAST_KNOWN_BLOCK && !testUnconfirmedTransactions) {
                    throw new HebeException.NotCurrentlyValidException("Blockchain not ready to accept transactions");
                }

                if (getUnconfirmedTransaction(transaction.getDbKey()) != null || TransactionDb.hasTransaction(transaction.getId())) {
                    throw new HebeException.ExistingTransactionException("Transaction already processed");
                }

                if (! transaction.verifySignature()) {
                    if (Account.getAccount(transaction.getSenderId()) != null) {
                        throw new HebeException.NotValidException("Transaction signature verification failed");
                    } else {
                        throw new HebeException.NotCurrentlyValidException("Unknown transaction sender");
                    }
                }

                if (! transaction.applyUnconfirmed()) {
                    throw new HebeException.InsufficientBalanceException("Insufficient balance");
                }

                if (transaction.isUnconfirmedDuplicate(unconfirmedDuplicates)) {
                    throw new HebeException.NotCurrentlyValidException("Duplicate unconfirmed transaction");
                }
     ...
     * @throws  HebeException.NotValidException    Transaction is not valid
     */
    @Override
    public List<Transaction> restorePrunableData(JSONArray transactions) throws HebeException.NotValidException {

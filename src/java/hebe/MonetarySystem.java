...
public abstract class MonetarySystem extends TransactionType {
    ...
    public static final TransactionType CURRENCY_ISSUANCE = new MonetarySystem() {
        ...
        private final Fee FIVE_LETTER_CURRENCY_ISSUANCE_FEE = new Fee.ConstantFee(40 * Constants.ONE_HEBE);
        private final Fee FOUR_LETTER_CURRENCY_ISSUANCE_FEE = new Fee.ConstantFee(1000 * Constants.ONE_HEBE);
        private final Fee THREE_LETTER_CURRENCY_ISSUANCE_FEE = new Fee.ConstantFee(25000 * Constants.ONE_HEBE);
        ...
        @Override
        Attachment.MonetarySystemCurrencyIssuance parseAttachment(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException {
            return new Attachment.MonetarySystemCurrencyIssuance(buffer, transactionVersion);
        }

        @Override
        Attachment.MonetarySystemCurrencyIssuance parseAttachment(JSONObject attachmentData) throws HebeException.NotValidException {
        ...
        void validateAttachment(Transaction transaction) throws HebeException.ValidationException {
            ...
            if (attachment.getMaxSupply() > Constants.MAX_CURRENCY_TOTAL_SUPPLY
                ...
                throw new HebeException.NotValidException("Invalid currency issuance: " + attachment.getJSONObject());
            }
            int t = 1;
            for (int i = 0; i < 32; i++) {
                if ((t & attachment.getType()) != 0 && CurrencyType.get(t) == null) {
                    throw new HebeException.NotValidException("Invalid currency type: " + attachment.getType());
                }
                t <<= 1;
            }
    ...
    public static final TransactionType RESERVE_INCREASE = new MonetarySystem() {
        ...
        @Override
        Attachment.MonetarySystemReserveIncrease parseAttachment(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException {
            return new Attachment.MonetarySystemReserveIncrease(buffer, transactionVersion);
        }

        @Override
        Attachment.MonetarySystemReserveIncrease parseAttachment(JSONObject attachmentData) throws HebeException.NotValidException {
        ...
        void validateAttachment(Transaction transaction) throws HebeException.ValidationException {
            ...
            if (attachment.getAmountPerUnitNQT() <= 0) {
                throw new HebeException.NotValidException("Reserve increase HEBE amount must be positive: " + attachment.getAmountPerUnitNQT());
            }
        ...
        void undoAttachmentUnconfirmed(Transaction transaction, Account senderAccount) {
            ...
            if (currency != null) {
                ...
                Transaction currencyIssuance = HEBE.getBlockchain().getTransaction(attachment.getCurrencyId());
    ...
    public static final TransactionType RESERVE_CLAIM = new MonetarySystem() {
        ...
        @Override
        Attachment.MonetarySystemReserveClaim parseAttachment(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException {
            return new Attachment.MonetarySystemReserveClaim(buffer, transactionVersion);
        }

        @Override
        Attachment.MonetarySystemReserveClaim parseAttachment(JSONObject attachmentData) throws HebeException.NotValidException {
        ...
        void validateAttachment(Transaction transaction) throws HebeException.ValidationException {
            ...
            if (attachment.getUnits() <= 0) {
                throw new HebeException.NotValidException("Reserve claim number of units must be positive: " + attachment.getUnits());
            }
            CurrencyType.validate(Currency.getCurrency(attachment.getCurrencyId()), transaction);
        }
    ...
    public static final TransactionType CURRENCY_TRANSFER = new MonetarySystem() {
        ...
        @Override
        Attachment.MonetarySystemCurrencyTransfer parseAttachment(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException {
            return new Attachment.MonetarySystemCurrencyTransfer(buffer, transactionVersion);
        }

        @Override
        Attachment.MonetarySystemCurrencyTransfer parseAttachment(JSONObject attachmentData) throws HebeException.NotValidException {
        ...
        void validateAttachment(Transaction transaction) throws HebeException.ValidationException {
            ...
            if (attachment.getUnits() <= 0) {
                throw new HebeException.NotValidException("Invalid currency transfer: " + attachment.getJSONObject());
            }
            if (transaction.getRecipientId() == Genesis.CREATOR_ID) {
                throw new HebeException.NotValidException("Currency transfer to genesis account not allowed");
            }
            Currency currency = Currency.getCurrency(attachment.getCurrencyId());
            CurrencyType.validate(currency, transaction);
            if (! currency.isActive()) {
                throw new HebeException.NotCurrentlyValidException("Currency not currently active: " + attachment.getJSONObject());
            }
        }
    ...
    public static final TransactionType PUBLISH_EXCHANGE_OFFER = new MonetarySystem() {
        ...
        @Override
        Attachment.MonetarySystemPublishExchangeOffer parseAttachment(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException {
            return new Attachment.MonetarySystemPublishExchangeOffer(buffer, transactionVersion);
        }

        @Override
        Attachment.MonetarySystemPublishExchangeOffer parseAttachment(JSONObject attachmentData) throws HebeException.NotValidException {
        ...
        void validateAttachment(Transaction transaction) throws HebeException.ValidationException {
            ...
            if (attachment.getBuyRateNQT() <= 0
                ...
                throw new HebeException.NotValidException(String.format("Invalid exchange offer, buy rate %d and sell rate %d has to be larger than 0, buy rate cannot be larger than sell rate",
                        attachment.getBuyRateNQT(), attachment.getSellRateNQT()));
            }
            if (attachment.getTotalBuyLimit() < 0
                ...
                throw new HebeException.NotValidException("Invalid exchange offer, units and height cannot be negative: " + attachment.getJSONObject());
            }
            if (attachment.getTotalBuyLimit() < attachment.getInitialBuySupply()
                ...
                throw new HebeException.NotValidException("Initial supplies must not exceed total limits");
            }
            if (HEBE.getBlockchain().getHeight() > Constants.SHUFFLING_BLOCK) {
                if (attachment.getTotalBuyLimit() == 0 && attachment.getTotalSellLimit() == 0) {
                    throw new HebeException.NotValidException("Total buy and sell limits cannot be both 0");
                }
                if (attachment.getInitialBuySupply() == 0 && attachment.getInitialSellSupply() == 0) {
                    throw new HebeException.NotValidException("Initial buy and sell supply cannot be both 0");
                }
            }
            if (attachment.getExpirationHeight() <= attachment.getFinishValidationHeight(transaction)) {
                throw new HebeException.NotCurrentlyValidException("Expiration height must be after transaction execution height");
            }
            Currency currency = Currency.getCurrency(attachment.getCurrencyId());
            CurrencyType.validate(currency, transaction);
            if (! currency.isActive()) {
    ...
    abstract static class MonetarySystemExchange extends MonetarySystem {
        ...
        final void validateAttachment(Transaction transaction) throws HebeException.ValidationException {
            ...
            if (attachment.getRateNQT() <= 0 || attachment.getUnits() == 0) {
                throw new HebeException.NotValidException("Invalid exchange: " + attachment.getJSONObject());
            }
            Currency currency = Currency.getCurrency(attachment.getCurrencyId());
            CurrencyType.validate(currency, transaction);
            if (! currency.isActive()) {
                throw new HebeException.NotCurrentlyValidException("Currency not active: " + attachment.getJSONObject());
            }
        }
    ...
    public static final TransactionType EXCHANGE_BUY = new MonetarySystemExchange() {
        ...
        @Override
        Attachment.MonetarySystemExchangeBuy parseAttachment(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException {
            return new Attachment.MonetarySystemExchangeBuy(buffer, transactionVersion);
        }

        @Override
        Attachment.MonetarySystemExchangeBuy parseAttachment(JSONObject attachmentData) throws HebeException.NotValidException {
        ...
        void applyAttachment(Transaction transaction, Account senderAccount, Account recipientAccount) {
            ...
            CurrencyExchangeOffer.exchangeHEBEForCurrency(transaction, senderAccount, attachment.getCurrencyId(), attachment.getRateNQT(), attachment.getUnits());
        }
    ...
    public static final TransactionType EXCHANGE_SELL = new MonetarySystemExchange() {
        ...
        @Override
        Attachment.MonetarySystemExchangeSell parseAttachment(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException {
            return new Attachment.MonetarySystemExchangeSell(buffer, transactionVersion);
        }

        @Override
        Attachment.MonetarySystemExchangeSell parseAttachment(JSONObject attachmentData) throws HebeException.NotValidException {
        ...
        void applyAttachment(Transaction transaction, Account senderAccount, Account recipientAccount) {
            Attachment.MonetarySystemExchangeSell attachment = (Attachment.MonetarySystemExchangeSell) transaction.getAttachment();
            ExchangeRequest.addExchangeRequest(transaction, attachment);
            CurrencyExchangeOffer.exchangeCurrencyForHEBE(transaction, senderAccount, attachment.getCurrencyId(), attachment.getRateNQT(), attachment.getUnits());
        }
    ...
    public static final TransactionType CURRENCY_MINTING = new MonetarySystem() {
        ...
        @Override
        Attachment.MonetarySystemCurrencyMinting parseAttachment(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException {
            return new Attachment.MonetarySystemCurrencyMinting(buffer, transactionVersion);
        }

        @Override
        Attachment.MonetarySystemCurrencyMinting parseAttachment(JSONObject attachmentData) throws HebeException.NotValidException {
        ...
        void validateAttachment(Transaction transaction) throws HebeException.ValidationException {
            ...
            CurrencyType.validate(currency, transaction);
            if (attachment.getUnits() <= 0) {
                throw new HebeException.NotValidException("Invalid number of units: " + attachment.getUnits());
            }
            if (attachment.getUnits() > (currency.getMaxSupply() - currency.getReserveSupply()) / Constants.MAX_MINTING_RATIO) {
                throw new HebeException.NotValidException(String.format("Cannot mint more than 1/%d of the total units supply in a single request", Constants.MAX_MINTING_RATIO));
            }
            if (!currency.isActive()) {
                throw new HebeException.NotCurrentlyValidException("Currency not currently active " + attachment.getJSONObject());
            }
            ...
            if (attachment.getCounter() <= counter) {
                throw new HebeException.NotCurrentlyValidException(String.format("Counter %d has to be bigger than %d", attachment.getCounter(), counter));
            }
            if (!CurrencyMinting.meetsTarget(transaction.getSenderId(), currency, attachment)) {
                throw new HebeException.NotCurrentlyValidException(String.format("Hash doesn't meet target %s", attachment.getJSONObject()));
            }
        }
    ...
    public static final TransactionType CURRENCY_DELETION = new MonetarySystem() {
        ...
        @Override
        Attachment.MonetarySystemCurrencyDeletion parseAttachment(ByteBuffer buffer, byte transactionVersion) throws HebeException.NotValidException {
            return new Attachment.MonetarySystemCurrencyDeletion(buffer, transactionVersion);
        }

        @Override
        Attachment.MonetarySystemCurrencyDeletion parseAttachment(JSONObject attachmentData) throws HebeException.NotValidException {
        ...
        void validateAttachment(Transaction transaction) throws HebeException.ValidationException {
            ...
            if (!currency.canBeDeletedBy(transaction.getSenderId())) {
                throw new HebeException.NotCurrentlyValidException("Currency " + Long.toUnsignedString(currency.getId()) + " cannot be deleted by account " +

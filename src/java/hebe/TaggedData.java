...
import hebe.db.VersionedPrunableDbTable;
import hebe.db.VersionedValuesDbTable;
...
public class TaggedData {
        ...
        protected void prune() {
            if (Constants.ENABLE_PRUNING) {
                try (Connection con = db.getConnection();
                    ...
                    int expiration = HEBE.getEpochTime() - Constants.MAX_PRUNABLE_LIFETIME;
    ...
    public static final class Tag {
        ...
        private static void add(TaggedData taggedData) {
            for (String tagValue : taggedData.getParsedTags()) {
                ...
                if (tag == null) {
                    tag = new Tag(tagValue, HEBE.getBlockchain().getHeight());
                }
                tag.count += 1;
                tagTable.insert(tag);
            }
    ...
    public TaggedData(Transaction transaction, Attachment.TaggedDataAttachment attachment) {
        this(transaction, attachment, HEBE.getBlockchain().getLastBlockTimestamp(), HEBE.getBlockchain().getHeight());
    }
    ...
    static void add(TransactionImpl transaction, Attachment.TaggedDataUpload attachment) {
        if (HEBE.getEpochTime() - transaction.getTimestamp() < Constants.MAX_PRUNABLE_LIFETIME && attachment.getData() != null) {
    ...
    static void extend(Transaction transaction, Attachment.TaggedDataExtend attachment) {
        ...
        if (HEBE.getEpochTime() - Constants.MAX_PRUNABLE_LIFETIME < timestamp.timestamp) {
            ...
            if (taggedData != null) {
                ...
                taggedData.blockTimestamp = HEBE.getBlockchain().getLastBlockTimestamp();
                taggedData.height = HEBE.getBlockchain().getHeight();
                taggedDataTable.insert(taggedData);
            }
        }
    }

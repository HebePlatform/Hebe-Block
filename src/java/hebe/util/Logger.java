...
/**
 * Handle logging for the HEBE node server
 */
public final class Logger {
     ...
     * The hebe/conf/logging-default.properties and hebe/conf/logging.properties configuration
    ...
    static {
        ...
        System.setProperty("java.util.logging.manager", "hebe.util.HebeLogManager");
        if (!(LogManager.getLogManager() instanceof HebeLogManager)) {
            System.setProperty("java.util.logging.manager",
                    (oldManager != null ? oldManager : "java.util.logging.LogManager"));
        }
        if (! Boolean.getBoolean("hebe.doNotConfigureLogging")) {
            try {
                Properties loggiheberoperties = new Properties();
                HEBE.loadProperties(loggiheberoperties, "logging-default.properties", true);
                HEBE.loadProperties(loggiheberoperties, "logging.properties", false);
                HEBE.updateLogFileHandler(loggiheberoperties);
                if (loggiheberoperties.size() > 0) {
                    ByteArrayOutputStream outStream = new ByteArrayOutputStream();
                    loggiheberoperties.store(outStream, "logging properties");
        ...
        log = org.slf4j.LoggerFactory.getLogger(hebe.HEBE.class);
        enableStackTraces = HEBE.getBooleanProperty("hebe.enableStackTraces");
        enableLogTraceback = HEBE.getBooleanProperty("hebe.enableLogTraceback");
    ...
    public static void shutdown() {
        if (LogManager.getLogManager() instanceof HebeLogManager) {
            ((HebeLogManager) LogManager.getLogManager()).hebeShutdown();
        }
    }

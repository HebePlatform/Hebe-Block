...
final class TransactionImpl implements Transaction {
    ...
    static final class BuilderImpl implements Builder {
        ...
        @Override
        public TransactionImpl build(String secretPhrase) throws HebeException.NotValidException {
        ...
        @Override
        public TransactionImpl build() throws HebeException.NotValidException {
            return build(null);
        }
    ...
    private TransactionImpl(BuilderImpl builder, String secretPhrase) throws HebeException.NotValidException {
        ...
        if (builder.feeNQT <= 0 || (Constants.correctInvalidFees && builder.signature == null)) {
            int effectiveHeight = (height < Integer.MAX_VALUE ? height : HEBE.getBlockchain().getHeight());
        ...
        if (builder.signature != null && secretPhrase != null) {
            throw new HebeException.NotValidException("Transaction is already signed");
            ...
            if (getSenderPublicKey() != null && ! Arrays.equals(senderPublicKey, Crypto.getPublicKey(secretPhrase))) {
                throw new HebeException.NotValidException("Secret phrase doesn't match transaction sender public key");
            }
...
//        return version;
        ...
        if (bytes == null) {
            try {
                ...
                if (useNQT()) {
                    ...
                    buffer.putInt((int) (amountNQT / Constants.ONE_HEBE));
                    buffer.putInt((int) (feeNQT / Constants.ONE_HEBE));
                    if (referencedTransactionFullHash != null) {
    ...
    static TransactionImpl.BuilderImpl newTransactionBuilder(byte[] bytes) throws HebeException.NotValidException {
        try {
            ...
            if (buffer.hasRemaining()) {
                throw new HebeException.NotValidException("Transaction bytes too long, " + buffer.remaining() + " extra bytes");
            }
            return builder;
        } catch (HebeException.NotValidException|RuntimeException e) {
            Logger.logDebugMessage("Failed to parse transaction bytes: " + Convert.toHexString(bytes));
            throw e;
        }
    }

    static TransactionImpl.BuilderImpl newTransactionBuilder(byte[] bytes, JSONObject prunableAttachments) throws HebeException.NotValidException {
    ...
    static TransactionImpl parseTransaction(JSONObject transactionData) throws HebeException.NotValidException {
        ...
        if (transaction.getSignature() != null && !transaction.checkSignature()) {
            throw new HebeException.NotValidException("Invalid transaction signature for transaction " + transaction.getJSONObject().toJSONString());
        }
        return transaction;
    }

    static TransactionImpl.BuilderImpl newTransactionBuilder(JSONObject transactionData) throws HebeException.NotValidException {
        try {
            ...
            if (transactionType == null) {
                throw new HebeException.NotValidException("Invalid transaction type: " + type + ", " + subtype);
            }
...
//                || HEBE.getBlockchain().getHeight() >= Constants.NQT_BLOCK);
    }
    ...
    @Override
    public void validate() throws HebeException.ValidationException {
    	// feeNQT <= 0 update
        if (timestamp == 0 ? (deadline != 0 || feeNQT != 0) : (deadline < 1 || feeNQT <= 0)
                ...
                || type == null) {
            throw new HebeException.NotValidException("Invalid transaction parameters:\n type: " + type + ", timestamp: " + timestamp
        ...
        if (referencedTransactionFullHash != null && referencedTransactionFullHash.length != 32) {
            throw new HebeException.NotValidException("Invalid referenced transaction full hash " + Convert.toHexString(referencedTransactionFullHash));
        }

        if (attachment == null || type != attachment.getTransactionType()) {
            throw new HebeException.NotValidException("Invalid attachment " + attachment + " for transaction of type " + type);
        }

        if (! type.canHaveRecipient()) {
            if (recipientId != 0 || getAmountNQT() != 0) {
                throw new HebeException.NotValidException("Transactions of this type must have recipient == 0, amount == 0");
            }
        }

        if (type.mustHaveRecipient() && version > 0) {
            if (recipientId == 0) {
                throw new HebeException.NotValidException("Transactions of this type must have a valid recipient");
            }
        }
        ...
        for (Appendix.AbstractAppendix appendage : appendages) {
            ...
            if (! appendage.verifyVersion(this.version)) {
                throw new HebeException.NotValidException("Invalid attachment version " + appendage.getVersion()
        ...
        if (getFullSize() > Constants.MAX_PAYLOAD_LENGTH) {
            throw new HebeException.NotValidException("Transaction size " + getFullSize() + " exceeds maximum payload size");
        }

        if (!validatingAtFinish) {
            int blockchainHeight = HEBE.getBlockchain().getHeight();
            long minimumFeeNQT = getMinimumFeeNQT(blockchainHeight);
            if (feeNQT < minimumFeeNQT) {
                throw new HebeException.NotCurrentlyValidException(String.format("Transaction fee %f HEBE less than minimum fee %f HEBE at height %d",
                        ((double) feeNQT) / Constants.ONE_HEBE, ((double) minimumFeeNQT) / Constants.ONE_HEBE, blockchainHeight));
            }
            if (blockchainHeight > Constants.FXT_BLOCK && ecBlockId != 0) {
                if (blockchainHeight < ecBlockHeight) {
                    throw new HebeException.NotCurrentlyValidException("ecBlockHeight " + ecBlockHeight
    ...
    private long getMinimumFeeNQT(int blockchainHeight) {
        ...
        if (referencedTransactionFullHash != null) {
            totalFee = Math.addExact(totalFee, Constants.ONE_HEBE);
        }
        return totalFee;
    }

...
echo "compiling hebe core..."
find src/java/hebe/ -name "*.java" > sources.tmp
javac -encoding utf8 -sourcepath "${SP}" -classpath "${CP}" -d classes/ @sources.tmp || exit 1
echo "hebe core class files compiled successfully"

echo "compiling hebe desktop..."
find src/java/hebedesktop/ -name "*.java" > sources.tmp
javac -encoding utf8 -sourcepath "${SP}" -classpath "${CP}" -d classes/ @sources.tmp
if [ $? -eq 0 ]; then
    echo "hebe desktop class files compiled successfully"
else
    echo "if javafx is not supported, hebe desktop compile errors are safe to ignore, but desktop wallet will not be available"
fi

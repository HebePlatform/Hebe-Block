...
jar/exe/dmg package, or run "java -jar hebe-client.jar" to start the
installer. 

Using the hebe-client.zip package:

Unpack the hebe-client.zip package and open a shell in the resulting hebe
...
the hebe_db blockchain database. The start.sh script can be run from any
directory, it starts the java process in the background and uses ~/.hebe for
configuration files, logs, and the hebe_db database. Unlike run.sh, start.sh
...
message "Hebe server 1.x.x started successfully" in the console log. If run in
desktop mode, a JavaFX window will open automatically. Otherwise, open a
browser, without stopping the java process, and go to http://localhost:8082 ,
where the Hebe UI should now be available.
...
to the Hebe installation directory, as the use of special characters may result
...
conf/hebe-default.properties file. All possible settings are listed, with
detailed explanation. If you decide to change any setting, do not edit
hebe-default.properties directly, but create a new conf/hebe.properties file
and only add to it the properties that need to be different from the default
values. You do not need to delete the defaults from hebe-default.properties, the
settings in hebe.properties override those in hebe-default.properties. This way,
when upgrading the software, you can safely overwrite hebe-default.properties
with the updated file from the new package, while your customizations remain
safe in the hebe.properties file.


How to contribute?

There are many ways to contribute to Hebe. Here are some examples:
...
The Hebe software is a client-server application. It consists of a java server
...
The user interface is available on port 8082. This port also accepts http API
requests which other Hebe client applications could use.

The blockchain is stored on disk using the H2 embedded database, inside the
hebe_db directory. When upgrading, you should not delete the old hebe_db
...
hebe_db, except that it will take some extra time to download the blockchain
from scratch.

The default Hebe client does not store any wallet-type file on disk. Unlike
...
which you see in the console window, but also to a file hebe.log, which gets
overwritten at restart. In case of an error, the hebe.log file may contain
helpful information, so include its contents when submitting a bug report.

In addition to the default user interface at http://localhost:8082 , the
following urls are available:

http://localhost:8082/test - a list of all available http API requests, very
...
http://localhost:8082/test?requestType=<specificRequestType> - same as above,
but only shows the form for the request type specified.

http://localhost:8082/doc - a javadoc documentation for client developers who
...
hebe.jar file using the enclosed jar.sh script, and then hebe.jar should be
